...
qx.Class.define(s,{extend:qx.application.Standalone,members:{__PW:null,__PX:null,main:function(){qx.application.Standalone.prototype.main.call(this);
var S=this.getRoot();
...
}var T=new qx.ui.form.CheckBox(q);
T.setValue(true);
...
var E=e.getCurrentType();
var B;

switch(E){case j:B=this.getSelection();
...
case k:B=this.getSelection()[0].getLabel();
break;
}if(C==h){var A=this.getSelection();
...
for(var i=0,l=W.length;i<l;i++){this.add(W[i]);
}});
L.addListener(f,function(e){if(!e.supportsType(j)){e.preventDefault();
}});
var N=new qx.ui.basic.Label(y);
...
var I=this.__PW=new qx.ui.form.List;
I.setDraggable(true);
I.setDroppable(true);
...
J.setDecorator(new qx.ui.decoration.Single().set({top:[1,x,u]}));
J.setHeight(0);
J.setOpacity(0.5);
J.setZIndex(100);
...
I.addListener(p,function(e){J.setDomPosition(-1000,-1000);
});
I.addListener(z,function(e){var G=e.getOriginalTarget();
if(G instanceof qx.ui.form.ListItem){qx.core.Init.getApplication().__PX=G;
}
if(!qx.ui.core.Widget.contains(this,G)&&G!=J){return;
}var F=G.getContainerLocation();
J.setWidth(G.getBounds().width);
J.setDomPosition(F.left,F.top);
});
I.addListener(f,function(e){if(e.getRelatedTarget()){e.preventDefault();
}});
I.addListener(m,function(e){this.__PY(e.getOriginalTarget());
},this);
J.addListener(m,function(e){this.__PY(this.__PX);
},this);
},__PY:function(U){if(U.classname!="qx.ui.form.ListItem"){return;
}var V=this.__PW.getSortedSelection();

for(var i=0,l=V.length;i<l;i++){this.__PW.addBefore(V[i],U);
this.__PW.addToSelection(V[i]);
