...
var A=new qx.ui.form.CheckBox(d);
A.setRequired(true);
this.getRoot().add(A,{left:20,top:190});
var x=new qx.ui.form.validation.Manager();
var v=function(C,D){var E=C!=null&&C.length>2;

if(!E){D.setInvalidMessage(k);
}return E;
};
var y=new qx.ui.form.validation.AsyncValidator(function(F,G){window.setTimeout(function(){if(G==null||G.length==0){F.setValid(false,i);
}else{F.setValid(true);
}},1000);
});
x.add(s,y);
...
x.add(u,v);
x.add(A);
x.setValidator(function(p){var r=t.getValue()==u.getValue();

if(!r){var q=h;
t.setInvalidMessage(q);
u.setInvalidMessage(q);
t.setValid(false);
u.setValid(false);
}return r;
});
...
w.addListener(b,function(){w.setEnabled(false);
w.setLabel(f);
x.validate();
},this);
x.addListener(l,function(){w.setEnabled(true);
w.setLabel(n);
