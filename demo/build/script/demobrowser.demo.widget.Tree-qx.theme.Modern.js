...
bD.add(bC);
return by;
},getCommandFrame:function(V){var bg=new qx.ui.groupbox.GroupBox(I);
var be=4;
bg.setLayout(new qx.ui.layout.Grid(5,3));
var bn=0;
bg.add(new qx.ui.basic.Label(o).set({paddingTop:4}),{row:bn,column:0});
var bk=new qx.ui.form.TextField().set({readOnly:true});
bg.add(bk,{row:bn++,column:1});
V.addListener(h,function(e){var Q=e.getData();

if(Q.length>0){if(this.getSelectionMode()===f){bk.setValue(Q.length+H);
}else{bk.setValue(Q[0].getLabel());
}}else{bk.setValue(t);
}});
bg.add(new qx.ui.core.Spacer(be,be),{row:bn++,column:0});
bg.add(new qx.ui.basic.Label(M),{row:bn,column:0});
var ba={"click":u,"dblclick":A,"none":y};
var X=new qx.ui.form.RadioGroup();

for(var bi in ba){var Y=new qx.ui.form.RadioButton(ba[bi]).set({value:bi==V.getOpenMode()});
Y.setUserData(g,bi);
X.add(Y);
bg.add(Y,{row:bn++,column:1});
}X.addListener(h,function(e){V.setOpenMode(e.getData()[0].getUserData(g));
});
bg.add(new qx.ui.core.Spacer(be,be),{row:bn++,column:0});
bg.add(new qx.ui.basic.Label(z),{row:bn,column:0});
var bc=new qx.ui.form.CheckBox(x);
bg.add(bc,{row:bn++,column:1});
bc.addListener(l,function(e){var P=e.getData();
V.setSelectionMode(P?f:d);
});
var bd=new qx.ui.form.CheckBox(r);
bg.add(bd,{row:bn++,column:1});
bd.addListener(l,function(e){var bo=e.getData();
V.setDragSelection(bo);

if(!bc.getValue()){bc.setValue(true);
}});
bg.add(new qx.ui.core.Spacer(be,be),{row:bn++,column:0});
bg.add(new qx.ui.basic.Label(B),{row:bn,column:0});
var bl=new qx.ui.form.CheckBox(K);
bg.add(bl,{row:bn++,column:1});
bl.addListener(l,function(e){V.setHideRoot(e.getData());
});
var bb=new qx.ui.form.CheckBox(v);
bg.add(bb,{row:bn++,column:1});
bb.addListener(l,function(e){V.setRootOpenClose(e.getData());
});
bg.add(new qx.ui.core.Spacer(be,be),{row:bn++,column:0});
var bj=new qx.ui.form.Button(D);
bg.add(bj,{row:bn++,column:1});
bj.addListener(k,function(e){alert((""+V.getItems()).replace(",","\n","g"));
});
var W=new qx.ui.form.Button(s);
bg.add(W,{row:bn++,column:1});
W.addListener(k,function(e){alert((""+V.getItems(true,false)).replace(",","\n","g"));
});
var bh=new qx.ui.form.Button(C);
bg.add(bh,{row:bn++,column:1});
bh.addListener(k,function(e){if(this.getSelectionMode()===d){alert(this.getSelection());
}else{alert(this.getSelection().toString().replace(",","\n","g"));
}},V);
var bf=new qx.ui.form.Button(w);
bg.add(bf,{row:bn++,column:1});
var bm=true;
bf.addListener(k,function(e){V.setHeight(bm?600:400);
bm=!bm;
});
return bg;
}}});
