...
try{Q.setName(p);
R.setHtml(R.getHtml()+i+Q.getName()+j);
}catch(W){R.setHtml(R.getHtml()+F+P+Q.getName()+j);
...
qx.Class.define(n,{extend:qx.core.Object,properties:{name:{init:v,validate:w},age:{init:18,validate:qx.util.Validate.number()},gender:{init:g,validate:qx.util.Validate.inArray([g,f])}},members:{__Pm:function(T){if(T.length<2){throw new qx.core.ValidationError(C+I+T+O);
}}}});
})();
