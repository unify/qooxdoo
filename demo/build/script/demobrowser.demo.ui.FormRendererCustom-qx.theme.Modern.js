...
r.add(v,c);
r.addGroupHeader(h);
r.add(new qx.ui.form.TextField(),e,qx.util.Validate.email());
r.add(new qx.ui.form.TextField(),d);
r.addGroupHeader(f);
r.add(new qx.ui.form.TextField(),g);
r.add(new qx.ui.form.TextArea(),k);
var t=new qx.ui.form.Button(j);
t.addListener(p,function(){if(r.validate()){alert("send...");
}},this);
r.addButton(t);
var s=new qx.ui.form.Button(a);
s.addListener(p,function(){r.reset();
},this);
r.addButton(s);
var u=new demobrowser.demo.ui.CustomRenderer(r);
this.getRoot().add(u,{left:10,top:10});
}}});
qx.Class.define(l,{extend:qx.ui.form.renderer.Single,members:{__Qa:0,__Qb:0,addItems:function(w,x,y){var B=0;
if(y!=null){this._add(this._createHeader(y),{row:B,column:this.__Qa,colSpan:2});
B++;
}for(var i=0;i<w.length;i++){var A=this._createLabel(x[i],w[i]);
this._add(A,{row:B,column:this.__Qa});
var z=w[i];
A.setBuddy(z);

if(z instanceof qx.ui.form.RadioGroup){z=this._createWidgetForRadioGroup(z);
}this._add(z,{row:B,column:this.__Qa+1});
B++;
}this.__Qa+=2;
this.__Qb<B?this.__Qb=B:null;
...
this._add(this._buttonRow,{row:this.__Qb,column:0,colSpan:this.__Qa});
