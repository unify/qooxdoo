...
qx.Class.define(c,{extend:qx.application.Standalone,include:demobrowser.demo.table.MUtil,members:{__Or:null,main:function(){qx.application.Standalone.prototype.main.call(this);
...
},showDialog:function(m){if(!this.__Or){var p=this.__Or=new qx.ui.window.Window().set({modal:true,showMinimize:false,showMaximize:false,width:180,contentPadding:[10,10,10,10]});
p.moveTo(315,100);
...
}this.__Or.getChildren()[1].setValue(m);
this.__Or.open();
this.__Or.getChildren()[2].focus();
...
(function(){var bm="dataChanged",bl="metaDataChanged",bk="Integer",bj="Boolean",bi="qx.ui.table.model.Remote";
...
this.__OO=null;
this.__OP=null;
},properties:{blockSize:{check:bk,init:50},maxCachedBlockCount:{check:bk,init:15},clearCacheOnRemove:{check:bj,init:false},blockConcurrentLoadRowCount:{check:bj,init:true}},members:{__OG:null,__OL:null,__OH:null,__OI:null,__OJ:null,__OK:null,__OM:null,__ON:null,__OE:null,__OF:null,__OP:null,__OO:null,__OQ:false,_getIgnoreCurrentRequest:function(){return this.__OL;
},getRowCount:function(){if(this.__OG==-1){if(!this.__OQ||!this.getBlockConcurrentLoadRowCount()){this.__OQ=true;
this._loadRowCount();
}return (this.__OG==-1)?0:this.__OG;
}else{return this.__OG;
}},_loadRowCount:function(){throw new Error("_loadRowCount is abstract");
},_onRowCountLoaded:function(bw){if(this.getBlockConcurrentLoadRowCount()){this.__OQ=false;
}if(bw==null||bw<0){bw=0;
}this.__OG=Number(bw);
var bx={firstRow:0,lastRow:bw-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(bm,bx);
},reloadData:function(){this.clearCache();
if(this.__OI!=-1){var bd=this._cancelCurrentRequest();
...
this.__OQ=true;
this._loadRowCount();
},clearCache:function(){this.__OM={};
this.__ON=0;
},getCacheContent:function(){return {sortColumnIndex:this.__OE,sortAscending:this.__OF,rowCount:this.__OG,lruCounter:this.__OH,rowBlockCache:this.__OM,rowBlockCount:this.__ON};
},restoreCacheContent:function(D){if(this.__OI!=-1){var E=this._cancelCurrentRequest();

if(E){this.__OI=-1;
this.__OL=false;
}else{this.__OL=true;
}}this.__OE=D.sortColumnIndex;
this.__OF=D.sortAscending;
this.__OG=D.rowCount;
this.__OH=D.lruCounter;
this.__OM=D.rowBlockCache;
this.__ON=D.rowBlockCount;
var F={firstRow:0,lastRow:this.__OG-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(bm,F);
},_cancelCurrentRequest:function(){return false;
},iterateCachedRows:function(J,K){var M=this.getBlockSize();
var L=Math.ceil(this.getRowCount()/M);
for(var T=0;T<=L;T++){var N=this.__OM[T];

if(N!=null){var S=T*M;
var R=N.rowDataArr;

for(var Q=0;Q<R.length;Q++){var P=R[Q];
var O=J.call(K,S+Q,P);

if(O!=null){R[Q]=O;
}}}}},prefetchRows:function(k,l){if(this.__OI==-1){var m=this.getBlockSize();
var r=Math.ceil(this.__OG/m);
var q=parseInt(k/m)-1;

if(q<0){q=0;
}var p=parseInt(l/m)+1;

if(p>=r){p=r-1;
}var o=-1;
var n=-1;

for(var s=q;s<=p;s++){if(this.__OM[s]==null||this.__OM[s].isDirty){if(o==-1){o=s;
}n=s;
}}if(o!=-1){this.__OJ=-1;
this.__OK=-1;
this.__OI=o;
this._loadRowData(o*m,(n+1)*m-1);
}}else{this.__OJ=k;
this.__OK=l;
}},_loadRowData:function(h,j){throw new Error("_loadRowCount is abstract");
},_onRowDataLoaded:function(U){if(U!=null&&!this.__OL){var X=this.getBlockSize();
var V=Math.ceil(U.length/X);

if(V==1){this._setRowBlockData(this.__OI,U);
}else{for(var i=0;i<V;i++){var bb=i*X;
var ba=[];
var W=Math.min(X,U.length-bb);

for(var bc=0;bc<W;bc++){ba.push(U[bb+bc]);
}this._setRowBlockData(this.__OI+i,ba);
}}var Y={firstRow:this.__OI*X,lastRow:(this.__OI+V+1)*X-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(bm,Y);
}this.__OI=-1;
this.__OL=false;
if(this.__OJ!=-1){this.prefetchRows(this.__OJ,this.__OK);
}},_setRowBlockData:function(t,u){if(this.__OM[t]==null){this.__ON++;

while(this.__ON>this.getMaxCachedBlockCount()){var y;
var x=this.__OH;

for(var w in this.__OM){var v=this.__OM[w].lru;

if(v<x&&w>1){x=v;
y=w;
}}delete this.__OM[y];
this.__ON--;
}}this.__OM[t]={lru:++this.__OH,rowDataArr:u};
...
if(bG==bC-1){if(bz.rowDataArr.length==0){delete this.__OM[bG];
}}else{var bE=this.__OM[bG+1];

if(bE!=null){bz.rowDataArr.push(bE.rowDataArr[0]);
}else{bz.isDirty=true;
}}}}
if(this.__OG!=-1){this.__OG--;
}if(this.hasListener(bm)){var bF={firstRow:by,lastRow:this.getRowCount()-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(bm,bF);
}}},getRowData:function(c){var d=this.getBlockSize();
var g=parseInt(c/d);
var e=this.__OM[g];

if(e==null){return null;
}else{var f=e.rowDataArr[c-(g*d)];
if(e.lru!=this.__OH){e.lru=++this.__OH;
}return f;
}},getValue:function(z,A){var B=this.getRowData(A);

if(B==null){return null;
}else{var C=this.getColumnId(z);
return B[C];
...
bq[bs]=bp;
if(this.hasListener(bm)){var br={firstRow:bo,lastRow:bo,firstColumn:bn,lastColumn:bn};
this.fireDataEvent(bm,br);
}}},setEditable:function(bu){this.__OP=[];

for(var bv=0;bv<this.getColumnCount();bv++){this.__OP[bv]=bu;
}this.fireEvent(bl);
},setColumnEditable:function(G,H){if(H!=this.isColumnEditable(G)){if(this.__OP==null){this.__OP=[];
}this.__OP[G]=H;
this.fireEvent(bl);
}},isColumnEditable:function(b){return (this.__OP?(this.__OP[b]==true):false);
},setColumnSortable:function(bg,bh){if(bh!=this.isColumnSortable(bg)){if(this.__OO==null){this.__OO=[];
}this.__OO[bg]=bh;
this.fireEvent(bl);
}},isColumnSortable:function(bt){return (this.__OO?(this.__OO[bt]!==false):true);
},sortByColumn:function(be,bf){if(this.__OE!=be||this.__OF!=bf){this.__OE=be;
this.__OF=bf;
this.clearCache();
this.fireEvent(bl);
}},getSortColumnIndex:function(){return this.__OE;
},isSortAscending:function(){return this.__OF;
},setSortColumnIndexWithoutSortingData:function(a){this.__OE=a;
},setSortAscendingWithoutSortingData:function(I){this.__OF=I;
}},destruct:function(){this.__OO=this.__OP=this.__OM=null;
}});
})();
