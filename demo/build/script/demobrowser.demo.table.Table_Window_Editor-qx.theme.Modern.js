...
var w={tableColumnModel:function(u){return new qx.ui.table.columnmodel.Resize(u);
}};
...
qx.Class.define(b,{extend:qx.core.Object,implement:qx.ui.table.ICellEditorFactory,members:{createCellEditor:function(p){var s=new qx.ui.window.Window(d);
s.setLayout(new qx.ui.layout.HBox(4));
s.set({padding:3,modal:true,showClose:false,showMaximize:false,showMinimize:false});
s.moveTo(300,250);
s.addListener(f,function(e){s.__OR.focus();
s.__OR.setTextSelection(0,s.__OR.getValue().length);
});
s.__OR=new qx.ui.form.TextField(p.value+"").set({allowGrowY:true});
s.add(s.__OR);
var q=new qx.ui.form.Button(a);
q.addListener(o,function(e){s.close();
});
s.add(q);
var r=new qx.ui.core.Command(j);
r.addListener(o,function(e){q.execute();
r.dispose();
r=null;
});
return s;
},getCellEditorValue:function(t){return parseFloat(t.__OR.getValue());
}}});
})();
