...
(function(){var d=".",c="boolean",b="qx.bom.client.Flash";
qx.Class.define(b,{statics:{AVAILABLE:false,FULLVERSION:"0.0.0",REVISION:"0",VERSION:0.0,EXPRESSINSTALL:false,STRICT_SECURITY_MODEL:false,_cachedSupportsVersion:{},supportsVersion:function(m){if(typeof this._cachedSupportsVersion[m]===c){return this._cachedSupportsVersion[m];
}else{var p=m.split(d);
var o=this.FULLVERSION.split(d);

for(var i=0;i<p.length;i++){var n=parseInt(o[i])-parseInt(p[i]);

if(n>0){return (this._cachedSupportsVersion[m]=true);
}else if(n<0){return (this._cachedSupportsVersion[m]=false);
}}return (this._cachedSupportsVersion[m]=true);
}},__LK:qx.core.Variant.select("qx.client",{"mshtml":function(){if(!window.ActiveXObject){return;
}var h=[0,0,0];
var g=false;

try{var j=new ActiveXObject("ShockwaveFlash.ShockwaveFlash.7");
}catch(a){try{var j=new ActiveXObject("ShockwaveFlash.ShockwaveFlash.6");
h=[6,0,21];
j.AllowScriptAccess="always";
}catch(e){if(h[0]==6){g=true;
}}
if(!g){try{j=new ActiveXObject("ShockwaveFlash.ShockwaveFlash");
}catch(t){}}}
if(!g&&typeof j=="object"){var f=j.GetVariable("$version");

if(typeof f!="undefined"){f=f.split(" ")[1].split(",");
h[0]=parseInt(f[0],10);
h[1]=parseInt(f[1],10);
h[2]=parseInt(f[2],10);
}}this.__LL(h);
...
k[0]=parseInt(l.replace(/^(.*)\..*$/,"$1"),10);
k[1]=parseInt(l.replace(/^.*\.(.*)\s.*$/,"$1"),10);
k[2]=/r/.test(l)?parseInt(l.replace(/^.*r(.*)$/,"$1"),10):0;
}this.__LL(k);
}}),__LL:function(r){this.FULLVERSION=r.join(".");
...
}},defer:function(q){q.__LK();
}});
})();
(function(){var a="qx.bom.client.Multimedia";
qx.Class.define(a,{statics:{__LM:{quicktime:{plugin:"QuickTime",control:"QuickTimeCheckObject.QuickTimeCheck.1"},wmv:{plugin:"Windows Media",control:"WMPlayer.OCX.7"},divx:{plugin:"DivX Web Player",control:"npdivx.DivXBrowserPlugin.1"},silverlight:{plugin:"Silverlight",control:"AgControl.AgControl"}},has:function(s,t){if(this.__LM[s]){var u=this.__LM[s];
if(u.installed){if(t==null||(u.version>=parseFloat(t))){return true;
}}}return false;
},__LN:qx.core.Variant.select("qx.client",{"mshtml":function(){var c=window.ActiveXObject;
...
}switch(g){case "quicktime":d.version=0;
break;
case "wmv":d.version=f.versionInfo;
break;
case "divx":d.version=0;
break;
case "silverlight":try{d.version=f.version===undefined?0:f.version;
}catch(q){try{d.version=f.settings.version===undefined?0:f.settings.version;
}catch(r){d.version=0;
}}break;
}d.installed=true;
}},"default":function(){var n=navigator.plugins;
...
for(var i=0,m=n.length;i<m;i++){j=n[i];
name=j.name;

for(var p in k){o=k[p];

if(!o.installed&&name.indexOf(o.plugin)!==-1){o.installed=true;

if(l.test(j.name)||l.test(j.description)){o.version=parseFloat(RegExp.$1,10);
}else{o.version=0;
}break;
}}}}})},defer:function(b){b.__LN();
}});
})();
