...
var W=new qx.ui.progressive.renderer.table.Widths(1);
W.setWidth(0,q);
var Q=new qx.ui.progressive.headfoot.Progress(W);
var U=new qx.ui.progressive.structure.Default(null,Q);
var P=new qx.ui.progressive.Progressive(U);
P.setFlushWidgetQueueAfterBatch(true);
var T=function(F){var G={renderer:p,data:F};
return G;
};
var bb=new qx.ui.progressive.model.Default();
var X=new qx.ui.progressive.renderer.FunctionCaller();
P.addRenderer(p,X);
var V=k;
var Y=new qx.ui.basic.Image(V,q,q);
P.add(Y);
P.set({height:100,width:272,zIndex:99999,backgroundColor:D,opacity:0.86,batchSize:10});
this.getRoot().add(P,{top:-1000,left:-1000});
var ba=[{text:l,background:v,color:r},{text:B,background:j,color:r},{text:z,background:C,color:r},{text:t,background:w,color:r},{text:A,background:i,color:m}];
this.context={document:this.getRoot(),pages:ba};
P.addListener(y,function(e){var f=e.getData().state;
...
P.addListener(u,function(e){qx.event.Timer.once(function(){this.getLayoutParent().remove(this);
this.dispose();
},this,0);
});
bb.addElement(T(function(bd){var be=bd.context;
be.tabview=new qx.ui.tabview.TabView();
be.tabview.set({});
be.document.add(be.tabview,{left:20,right:20,top:20,bottom:20});
be.page=0;
be.currentPage=null;
}));

for(var R=0;R<ba.length;R++){bb.addElement(T(function(J){var L=J.context;
var K=L.pages[L.page];
L.currentPage=new qx.ui.tabview.Page(K.text);
L.currentPage.setLayout(new qx.ui.layout.VBox(4));
L.tabview.add(L.currentPage);
L.row=0;
L.col=0;
if(L.page==0){L.firstPage=L.currentPage;
}}));
for(var bc=0;bc<20;bc++){bb.addElement(T(function(bf){var bg=bf.context;
...
for(var S=0;S<10;S++){bb.addElement(T(function(M){var O=M.context;
var N=O.pages[O.page];
var o=new qx.ui.basic.Atom(s+O.row+x+h+O.col);
o.set({width:90,backgroundColor:N.background,textColor:N.color});
O.hBox.add(o);
}));
bb.addElement(T(function(E){++E.context.col;
}));
}bb.addElement(T(function(bh){++bh.context.row;
}));
}bb.addElement(T(function(I){++I.context.page;
}));
}bb.addElement(T(function(H){H.context.tabview.setSelection([H.context.firstPage]);
}));
P.setDataModel(bb);
P.render();
}}});
