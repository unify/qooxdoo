...
qx.Class.define(a,{extend:qx.fx.Base,properties:{mode:{init:d,check:[d,b]},x:{init:0,check:c},y:{init:0,check:c}},members:{__Lp:null,__Lq:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var e=this._getElement();
this.__Lp={x:e.scrollLeft,y:e.scrollTop};
if(this._atEndPosition(this.__Lp.x,this.__Lp.y)){return;
}
if(this.getMode()==b){this.__Lq={left:this.getX()-this.__Lp.x,top:this.getY()-this.__Lp.y};
}},update:function(h){qx.fx.Base.prototype.update.call(this);
var i=this._getElement();

if(this.getMode()==d){if(this.getX()!=0){i.scrollLeft=this.__Lp.x+(this.getX()*h);
}
if(this.getY()!=0){i.scrollTop=this.__Lp.y+(this.getY()*h);
}}else{i.scrollLeft=this.__Lp.x+(this.__Lq.left*h);
i.scrollTop=this.__Lp.y+(this.__Lq.top*h);
