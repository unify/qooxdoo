...
qx.util.Permutation.permute(j,function(p){var q=l.createHeaderCell(p);
k.add(q);
l.updateHeaderCell(p,q);
});
},testIconRenderer:function(){var t=new qx.ui.table.headerrenderer.Icon(f);
t.setToolTip(g);
var r={name:[i],col:[1],xPos:[2],editable:[true,false],sorted:[true,false],sortedAscending:[true,false]};
var s=this._getNewHeaderContainer();
qx.util.Permutation.permute(r,function(m){var n=t.createHeaderCell(m);
s.add(n);
t.updateHeaderCell(m,n);
});
},_getNewHeaderContainer:function(){var o=new qx.ui.container.Composite(new qx.ui.layout.HBox(5));
o.addListener(d,function(e){e.getTarget().addState(h);
});
o.addListener(a,function(e){e.getTarget().removeState(h);
});
