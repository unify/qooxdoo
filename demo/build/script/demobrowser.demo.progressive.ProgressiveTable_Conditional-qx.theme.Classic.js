...
qx.Class.define(d,{extend:qx.ui.progressive.renderer.table.cell.Abstract,construct:function(w,x,y,z){qx.ui.progressive.renderer.table.cell.Abstract.call(this);
this.__NN=["==","!=",">","<",">=","<="];
this.__NO=["between","!between"];
this.__NP=[];
this.__NQ=w||"";
this.__NR=x||"";
this.__NS=y||"";
this.__NT=z||"";
},members:{__NN:null,__NO:null,__NP:null,__NQ:null,__NR:null,__NS:null,__NT:null,__NU:function(Q,R){if(Q.align){R["text-align"]=Q.align;
}
if(Q.color){R["color"]=Q.color;
}
if(Q.style){R["font-style"]=Q.style;
}
if(Q.weight){R["font-weight"]=Q.weight;
}},addNumericCondition:function(p,q,r,s,t,u,v){if(!qx.lang.Array.contains(this.__NN,p)||q==null){throw new Error("Condition not recognized or value is null!");
}this.__NP.push({condition:p,align:r,color:s,style:t,weight:u,value1:q,target:v});
},addBetweenCondition:function(I,J,K,L,M,N,O,P){if(!qx.lang.Array.contains(this.__NO,I)||J==null||K==null){throw new Error("Condition not recognized or value1/value2 is null!");
}this.__NP.push({condition:I,align:L,color:M,style:N,weight:O,value1:J,value2:K,target:P});
},addRegex:function(S,T,U,V,W,X){if(!S){throw new Error("regex cannot be null!");
}this.__NP.push({condition:n,align:T,color:U,style:V,weight:W,regex:S,target:X});
},_getCellStyle:function(A){if(this.__NP.length==0){return A.style||o;
}var i;
var E;
var B;
var D={"text-align":this.__NQ,"color":this.__NR,"font-style":this.__NS,"font-weight":this.__NT};

for(i=0;i<this.__NP.length;i++){var H=this.__NP[i];
E=false;

if(qx.lang.Array.contains(this.__NN,H.condition)){if(H.target==null){B=A.cellData;
}else{B=A.element.data[H.target];
}
switch(H.condition){case m:if(B==H.value1){E=true;
}break;
case c:if(B!=H.value1){E=true;
}break;
case l:if(B>H.value1){E=true;
}break;
case h:if(B<H.value1){E=true;
}break;
case b:if(B>=H.value1){E=true;
}break;
case a:if(B<=H.value1){E=true;
}break;
}}else if(qx.lang.Array.contains(this.__NO,H.condition)){if(H.target==null){B=A.cellData;
}else{B=A.element.data[H.target];
}
switch(H.condition){case j:if(B>=H.value1&&B<=H.value2){E=true;
}break;
case k:if(B<H.value1&&B>H.value2){E=true;
}break;
}}else if(H.condition==n){if(H.target==null){B=A.cellData;
}else{B=A.element.data[H.target];
}var C=new RegExp(H.value1,g);
E=C.test(B);
}if(E){this.__NU(H,D);
}var F=[];

for(var G in D){if(D[G]){F.push(G,f,D[G],e);
}}}return F.join(o);
