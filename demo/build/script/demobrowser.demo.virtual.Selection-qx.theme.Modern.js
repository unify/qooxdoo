...
(function(){var p="manager",o="multi",n=" / ",m="bold",l="value",k="white",j="selected",i="#EEE",h="horizontal",g="table-scroller-header",K="one",J="single",I="changeValue",H="table-row-background-selected",G="additive",F="default",E="Additive Selection",D="Cell Line Selection",C="addaptive",B="Options",w="Row Selection",x="changeSelection",u="Column Selection",v="Enable drag selection",s="demobrowser.demo.virtual.Selection",t="Selection Mode",q="Single Selection",r="Cell Rectangle Selection",y="Enable quick selection",z="One Selection",A="Multi Selection";
...
var Q=qx.theme.manager.Font.getInstance().resolve(F).getStyles();
this._fontCss=qx.bom.element.Style.compile(Q);
var ba=new qx.ui.layout.Grid(5,0);
ba.setRowFlex(2,1);
ba.setColumnFlex(0,1);
ba.setColumnFlex(1,1);
ba.setColumnFlex(2,1);
ba.setColumnFlex(3,1);
var N=[];
var X=new qx.ui.container.Composite(ba);
this.getRoot().add(X,{edge:5});
X.add(new qx.ui.basic.Label(w).set({font:m,decorator:g,padding:3,allowGrowX:true}),{row:1,column:0});
var P=this.createRowSelectionScroller();
N.push(P.getUserData(p));
X.add(P,{row:2,column:0});
X.add(new qx.ui.basic.Label(u).set({font:m,decorator:g,padding:3,allowGrowX:true}),{row:1,column:1});
var P=this.createColumnSelectionScroller();
N.push(P.getUserData(p));
X.add(P,{row:2,column:1});
X.add(new qx.ui.basic.Label(r).set({font:m,decorator:g,padding:3,allowGrowX:true}),{row:1,column:2});
var P=this.createCellRectangleSelectionScroller();
N.push(P.getUserData(p));
X.add(P,{row:2,column:2});
X.add(new qx.ui.basic.Label(D).set({font:m,decorator:g,padding:3,allowGrowX:true}),{row:1,column:3});
var P=this.createCellLineSelectionScroller();
N.push(P.getUserData(p));
X.add(P,{row:2,column:3});
var V=new qx.ui.layout.Grid(20,4);
var S=new qx.ui.container.Composite(V).set({padding:15,backgroundColor:k});
X.add(S,{row:0,column:0,colSpan:4});
S.add(new qx.ui.basic.Label(t).set({font:m,padding:3}),{row:0,column:0});
var R=new qx.ui.form.RadioButton(q);
R.setUserData(l,J);
S.add(R,{row:1,column:0});
var U=new qx.ui.form.RadioButton(A);
U.setUserData(l,o);
U.setValue(true);
S.add(U,{row:2,column:0});
var O=new qx.ui.form.RadioButton(E);
O.setUserData(l,G);
S.add(O,{row:3,column:0});
var M=new qx.ui.form.RadioButton(z);
M.setUserData(l,K);
S.add(M,{row:4,column:0});
var T=new qx.ui.form.RadioGroup(R,U,O,M);
T.addListener(x,function(e){var L=e.getData()[0].getUserData(l);
N.forEach(function(bS){bS.setMode(L);
});

if(L==J||L==K){Y.setEnabled(false);
W.setEnabled(true);
}else if(L==o||L==C){Y.setEnabled(true);
W.setEnabled(false);
}});
S.add(new qx.ui.basic.Label(B).set({font:m,padding:3}),{row:0,column:1});
var Y=new qx.ui.form.CheckBox(v);
S.add(Y,{row:1,column:1});
Y.addListener(I,function(e){if(e.getData()){var bu=N[0].getMode();

if(bu==J||bu==K){this.debug("Drag selection is only available for the modes multi or additive");
}}N.forEach(function(bD){bD.setDrag(e.getData());
});
});
var W=new qx.ui.form.CheckBox(y).set({enabled:false});
S.add(W,{row:2,column:1});
W.addListener(I,function(e){if(e.getData()){var bh=N[0].getMode();

if(bh==o||bh==G){this.debug("Quick selection is only available for the modes multi or additive");
}}N.forEach(function(bo){bo.setQuick(e.getData());
});
});
},createRowSelectionScroller:function(){var bC=new qx.ui.virtual.core.Scroller(1000,100,20,100);
var bA=new qx.ui.virtual.layer.Row(k,i);
bC.getPane().addLayer(bA);
bC.getPane().addLayer(new qx.ui.virtual.layer.GridLines(h));
var bB=new qx.ui.virtual.cell.Cell();
var bz=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bP,bQ){var bR={};

if(by.isItemSelected(bP)){bR.selected=true;
}return bB.getCellProperties(bP+n+bQ,bR);
}});
bC.getPane().addLayer(bz);
var by=new qx.ui.virtual.selection.Row(bC.getPane(),{styleSelectable:function(bv,bw,bx){if(bw!==j){return;
}
if(bx){bA.setBackground(bv,j);
}else{bA.setBackground(bv,null);
}bz.updateLayerData();
}});
by.attachMouseEvents(bC.getPane());
by.attachKeyEvents(bC);
by.set({mode:o});
by.addItem(0);
bC.setUserData(p,by);
return bC;
},createColumnSelectionScroller:function(){var bs=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bs.getPane().addLayer(new qx.ui.virtual.layer.Row(k,i));
var br=new qx.ui.virtual.layer.Column();
bs.getPane().addLayer(br);
bs.getPane().addLayer(new qx.ui.virtual.layer.GridLines(h));
var bt=new qx.ui.virtual.cell.Cell();
var bq=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bl,bm){var bn={};

if(bp.isItemSelected(bm)){bn.selected=true;
}return bt.getCellProperties(bl+n+bm,bn);
}});
bs.getPane().addLayer(bq);
var bp=new qx.ui.virtual.selection.Column(bs.getPane(),{styleSelectable:function(bb,bc,bd){if(bc!==j){return;
}
if(bd){br.setBackground(bb,j);
}else{br.setBackground(bb,null);
}bq.updateLayerData();
}});
bp.attachMouseEvents(bs.getPane());
bp.attachKeyEvents(bs);
bp.set({mode:o});
bp.addItem(0);
bs.setUserData(p,bp);
return bs;
},createCellRectangleSelectionScroller:function(){var d=new qx.ui.virtual.core.Scroller(1000,100,20,100);
d.getPane().addLayer(new qx.ui.virtual.layer.Row(k,i));
var c=new qx.ui.virtual.cell.Cell();
var f=new qx.ui.virtual.cell.Cell().set({backgroundColor:H});
var b=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(bE,bF){var bG={};

if(a.isItemSelected({row:bE,column:bF})){bG.selected=true;
}
if(bG.selected){return f.getCellProperties(bE+n+bF,bG);
}else{c.resetBackgroundColor();
return c.getCellProperties(bE+n+bF,bG);
}}});
d.getPane().addLayer(b);
d.getPane().addLayer(new qx.ui.virtual.layer.GridLines(h));
var a=new qx.ui.virtual.selection.CellRectangle(d.getPane(),{styleSelectable:function(bi,bj,bk){b.updateLayerData();
}});
...
d.setUserData(p,a);
return d;
},createCellLineSelectionScroller:function(){var bN=new qx.ui.virtual.core.Scroller(1000,100,20,100);
bN.getPane().addLayer(new qx.ui.virtual.layer.Row(k,i));
var bM=new qx.ui.virtual.cell.Cell();
var bO=new qx.ui.virtual.cell.Cell().set({backgroundColor:H});
var bL=new qx.ui.virtual.layer.HtmlCell({getCellProperties:function(be,bf){var bg={};

if(bK.isItemSelected({row:be,column:bf})){bg.selected=true;
}
if(bg.selected){return bO.getCellProperties(be+n+bf,bg);
}else{bM.resetBackgroundColor();
return bM.getCellProperties(be+n+bf,bg);
}}});
bN.getPane().addLayer(bL);
bN.getPane().addLayer(new qx.ui.virtual.layer.GridLines(h));
var bK=new qx.ui.virtual.selection.CellLines(bN.getPane(),{styleSelectable:function(bH,bI,bJ){bL.updateLayerData();
}});
bK.attachMouseEvents(bN.getPane());
bK.attachKeyEvents(bN);
bK.set({mode:o});
bK.addItem({row:0,column:0});
bN.setUserData(p,bK);
return bN;
}}});
})();
...
},_scrollItemIntoView:function(a){this._pane.scrollColumnIntoView(a);
...
if(!J){return null;
}return this._isSelectable(J)?J:null;
},getSelectables:function(g){var i=[];
...
},_getSelectableRange:function(z,A){var D=[];
var E=Math.min(z.row,A.row);
var F=Math.max(z.row,A.row);
var C=Math.min(z.column,A.column);
var H=Math.max(z.column,A.column);

for(var I=E;I<=F;I++){for(var G=C;G<=H;G++){var B={row:I,column:G};

if(this._isSelectable(B)){D.push(B);
}}}return D;
},_getFirstSelectable:function(){var M=this._pane.getRowConfig().getItemCount();
var L=this._pane.getColumnConfig().getItemCount();

for(var P=0;P<M;P++){for(var O=0;O<L;O++){var N={row:P,column:O};

if(this._isSelectable(N)){return N;
}}}return null;
},_getLastSelectable:function(){var v=this._pane.getRowConfig().getItemCount();
var u=this._pane.getColumnConfig().getItemCount();

for(var x=u-1;x>=0;x--){for(var y=v-1;y>=0;y--){var w={row:y,column:x};

if(this._isSelectable(w)){return w;
}}}return null;
},_getRelatedSelectable:function(Q,R){var S={row:Q.row,column:Q.column};

switch(R){case e:for(var W=Q.row-1;W>=0;W--){S.row=W;

if(this._isSelectable(S)){return S;
}}break;
case f:var T=this._pane.getRowConfig().getItemCount();

for(var W=Q.row+1;W<T;W++){S.row=W;

if(this._isSelectable(S)){return S;
}}break;
case d:for(var V=Q.column-1;V>=0;V--){S.column=V;

if(this._isSelectable(S)){return S;
}}break;
case c:var U=this._pane.getColumnConfig().getItemCount();

for(var V=Q.column+1;V<U;V++){S.column=V;

if(this._isSelectable(S)){return S;
}}break;
}return null;
},_getPage:function(s,t){if(t){return this._getFirstSelectable();
}else{return this._getLastSelectable();
}},_selectableToHashCode:function(r){return r.column+b+r.row;
},_scrollItemIntoView:function(K){this._pane.scrollCellIntoView(K.column,K.row);
