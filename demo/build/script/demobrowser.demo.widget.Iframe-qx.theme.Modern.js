...
qx.Class.define(j,{extend:qx.ui.embed.AbstractIframe,construct:function(T){qx.ui.embed.AbstractIframe.call(this,T);
var U=new qx.ui.layout.Grid();
U.setColumnFlex(0,1);
U.setRowFlex(0,1);
this._setLayout(U);
this._showChildControl(v);
},properties:{appearance:{refine:true,init:d},scrollbarX:{check:[z,t,h],init:z,themeable:true,apply:p},scrollbarY:{check:[z,t,h],init:z,themeable:true,apply:p},scrollbar:{group:[i,l]}},members:{__Rb:null,__Rc:null,_getIframeElement:function(){return this.getChildControl(v).getContentElement();
},_createChildControlImpl:function(R){var S;

switch(R){case v:S=new qx.ui.embed.Iframe(this.getSource());
S.addListener(o,this._onIframeLoad,this);
S.addListener(n,this._onIframeResize,this);
this._add(S,{row:0,column:0});
break;
case s:S=new qx.ui.core.scroll.ScrollBar(g);
S.setMinWidth(0);
S.exclude();
S.addListener(r,this._onScrollBarX,this);
this._add(S,{row:1,column:0});
break;
case w:S=new qx.ui.core.scroll.ScrollBar(m);
S.setMinHeight(0);
S.exclude();
S.addListener(r,this._onScrollBarY,this);
this._add(S,{row:0,column:1});
break;
case u:S=new qx.ui.core.Widget();
S.setWidth(0);
S.setHeight(0);
S.exclude();
...
},_addWheelListener:function(){try{var Y=this._getIframeElement().getBody();
qx.event.Registration.addListener(Y,f,this._onMouseWheel,this);
}catch(e){this._disableScollbars();
}},_onMouseWheel:function(e){var b=this._isChildControlVisible(w)&&this.getChildControl(w).isEnabled();

if(!b){return;
}var a=this.getChildControl(w,true);
a.scrollBySteps(e.getWheelDelta());
e.stop();
},_startIframeObserver:function(){if(this.__Rc){this._stopIframeObserver();
}var V=qx.event.Idle.getInstance();
this.__Rc=V.addListener(k,this._onIframeObserverInterval,this);
},_stopIframeObserver:function(){this.__Rb=null;

if(!this.__Rc){return;
}var X=qx.event.Idle.getInstance();
X.removeListenerById(this.__Rc);
},_onIframeObserverInterval:function(){var c=this._getIframeSize();

if(!c){this._disableScollbars();
return;
}
if(this.__Rb&&c.width==this.__Rb.width&&c.height==this.__Rb.height){return;
}this.__Rb=c;
this._preventIframeScrolling();
this._updateScrollbars();
},_preventIframeScrolling:function(){try{var O=this._getIframeElement().getDocument();
O.documentElement.style.overflow=q;
O.body.style.overflow=q;
}catch(e){this._disableScollbars();
}},_updateScrollbars:function(){var B=this.__Rb;
var E=this.getChildControl(v).getBounds();
var C=this.getChildControl(v).getInnerSize();
...
if(F===z&&G===z){var D=B.width>C.width;
var H=B.height>C.height;
if((D||H)&&!(D&&H)){if(D){H=B.height>E.height;
}else if(H){D=B.width>E.width;
}}}else{var D=F===t;
var H=G===t;
if(B.width>(D?E.width:C.width)&&F===z){D=true;
}
if(B.height>(D?E.height:C.height)&&G===z){H=true;
}}this._configureScrollbar(s,D,C.width,B.width);
this._configureScrollbar(w,H,C.height,B.height);
...
}},_configureScrollbar:function(I,J,K,L){if(!J){this._excludeChildControl(I);
return;
}var M=this._showChildControl(I);

if(K>=L){M.set({position:0,maximum:L,knobFactor:1,enabled:false});
}else{M.setMaximum(1000000);
M.set({position:Math.min(M.getPosition(),L),maximum:L-K,knobFactor:K/L,enabled:true});
}},_onScrollBarX:function(e){this.scrollToX(e.getData());
},_onScrollBarY:function(e){this.scrollToY(e.getData());
},scrollToX:function(x){try{var A=this._getIframeElement().getWindow();
A.scroll(x,qx.bom.Viewport.getScrollTop(A));
}catch(e){this._disableScollbars();
}},scrollToY:function(y){try{var W=this._getIframeElement().getWindow();
W.scroll(qx.bom.Viewport.getScrollLeft(W),y);
}catch(e){this._disableScollbars();
