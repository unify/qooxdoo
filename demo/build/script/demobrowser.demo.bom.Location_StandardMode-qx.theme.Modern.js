...
var q=document.getElementById(j).getElementsByTagName(d);
...
}function s(o,p){return parseInt(qx.bom.element.Style.get(o,p,qx.bom.element.Style.COMPUTED_MODE,false),10);
}var u=this.href;
var w=u.substr(u.indexOf(h)+1);
var t=document.getElementById(w);
var location=qx.bom.element.Location.get(t,document.getElementById(g).value);
...
}else{v.style.width=(location.right-location.left-s(v,e)-s(v,a))+n;
v.style.height=(location.bottom-location.top-s(v,f)-s(v,k))+n;
}return false;
},randomize:function(){var r=document.body.getElementsByTagName(c);

for(var i=0,l=r.length;i<l;i++){if(r[i].id&&r[i].id!=m){r[i].style.margin=Math.round(Math.random()*10)+n;
r[i].style.padding=Math.round(Math.random()*10)+n;
r[i].style.borderWidth=Math.round(Math.random()*10)+n;
}}}}});
})();
