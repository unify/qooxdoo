...
},createTable:function(){var I=this.createRandomRows(50);
var H=this._tableModel=new qx.ui.table.model.Simple();
H.setColumns([m,b,h,g]);
H.setData(I);
H.setColumnEditable(1,true);
H.setColumnEditable(2,true);
H.setColumnSortable(3,false);
var J=new qx.ui.table.Table(H);
J.set({width:600,height:400,decorator:null});
J.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
var G=J.getTableColumnModel();
G.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
G.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(a,h));
return J;
},createControls:function(){var E=new qx.ui.toolbar.ToolBar();
var C,D,F;
D=new qx.ui.toolbar.Part();
E.add(D);
C=new qx.ui.toolbar.Button(c,f);
C.addListener(k,function(Q){var R=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,R[0][i]);
}this.info("Row 10 changed");
},this);
D.add(C);
C=new qx.ui.toolbar.Button(z,r);
C.addListener(k,function(bc){var bd=this.createRandomRows(10);
this._tableModel.addRows(bd);
this.info("10 rows added");
},this);
D.add(C);
C=new qx.ui.toolbar.Button(A,u);
C.addListener(k,function(be){var bf=this._tableModel.getRowCount();
this._tableModel.removeRows(bf-5,5);
this.info("5 rows removed");
},this);
D.add(C);
C=new qx.ui.toolbar.Button(o,d);
C.addListener(k,function(ba){var bb=[];
B.getSelectionModel().iterateSelection(function(P){bb.push(P+y);
});
this.showDialog(p+bb.join(s));
},this);
D.add(C);
D=new qx.ui.toolbar.Part();
E.add(D);
var B=this._table;
F=new qx.ui.toolbar.CheckBox(q);
F.set({value:B.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(t+n)});
F.addListener(j,function(K){B.setKeepFirstVisibleRowComplete(this.getValue());
},F);
D.add(F);
F=new qx.ui.toolbar.CheckBox(e);
F.set({value:B.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(l)});
F.addListener(j,function(S){if(S.getData()){var U=function(V,W){var X=V[arguments.callee.columnIndex];
...
var T=function(L,M){var N=L[arguments.callee.columnIndex];
var O=M[arguments.callee.columnIndex];

if(N%2==1&&O%2==0){return -1;
}
if(O%2==1&&N%2==0){return 1;
}return (N<O)?1:((N==O)?0:-1);
};
B.getTableModel().setSortMethods(0,{ascending:U,descending:T});
}else{B.getTableModel().setSortMethods(0,null);
}},F);
D.add(F);
return E;
