...
(function(){var w="Remove ",v="_gridWidgets",u="yellow",t="demobrowser.demo.test.ChildrenHandling",s="Horizontal box layout:",r="blue",q="Add ",p="_widgets",o="_hbox",n="green",j="Grid layout:",m="black",l="orange",h="_grid",g="red",k="execute";
...
e.setSpacing(10);
var b=new qx.ui.container.Composite(e).set({padding:10});
b.add(new qx.ui.basic.Label(s));
this._hbox=new qx.ui.container.Composite(new qx.ui.layout.HBox()).set({height:100});
this._widgets=[];
var f=[n,r,u,m,l,g];

for(var i=0;i<f.length;i++){var a=new qx.ui.core.Widget();
a.setBackgroundColor(f[i]);
this._hbox.add(a);
this._widgets.push(a);
}b.add(this._hbox);
b.add(new qx.ui.basic.Label(j));
var d=new qx.ui.layout.Grid();
d.setRowFlex(0,1);
this._grid=new qx.ui.container.Composite(d).set({height:100});
this._gridWidgets=[];

for(var i=0;i<f.length;i++){var a=new qx.ui.core.Widget();
a.setBackgroundColor(f[i]);
this._grid.add(a,{row:0,column:i});
this._gridWidgets.push(a);
}b.add(this._grid);
var c=new qx.ui.container.Composite(new qx.ui.layout.HBox());

for(var i=0;i<this._widgets.length;i++){c.add(this.createRemoveButton(this._widgets[i],i));
}b.add(c);
this.getRoot().add(b,{left:0,top:0});
},createRemoveButton:function(x,y){var A=true;
var z=new qx.ui.form.Button(w+x.getBackgroundColor()).set({width:100});
z.addListener(k,function(){if(A){this._hbox.remove(x);
...
z.setLabel(w+x.getBackgroundColor());
}A=!A;
},this);
return z;
}},destruct:function(){this._disposeObjects(o,h);
