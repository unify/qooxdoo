...
qx.Class.define(t,{extend:qx.core.Object,construct:function(a){qx.core.Object.call(this);
this.setQueue(qx.fx.queue.Manager.getInstance().getDefaultQueue());
this.__vz=qx.fx.Base.EffectState.IDLE;
this.__vA=a;
},events:{"setup":A,"update":A,"finish":A},properties:{duration:{init:0.5,check:B,apply:o},fps:{init:100,check:B},sync:{init:false,check:w},from:{init:0,check:B},to:{init:1,check:B},delay:{init:0.0,check:B},queue:{check:q},transition:{init:y,check:[y,n,k,s,x,l,p,r,j,u,m]}},statics:{EffectState:{IDLE:'idle',PREPARING:'preparing',FINISHED:'finished',RUNNING:'running'}},members:{__vz:null,__vB:null,__vC:null,__vD:null,__vE:null,__vF:null,__vG:null,__vH:null,__vA:null,_getElement:function(){return this.__vA;
},_setElement:function(b){this.__vA=b;
},_applyDuration:function(g,h){},init:function(){this.__vz=qx.fx.Base.EffectState.PREPARING;
this.__vB=0;
...
},update:function(f){},finish:function(){this.fireEvent(i);
},start:function(){if(this.__vz!=qx.fx.Base.EffectState.IDLE){return false;
}this.init();
...
this.afterFinish();
},render:function(C){if(this.__vz==qx.fx.Base.EffectState.PREPARING){this.__vz=qx.fx.Base.EffectState.RUNNING;
...
if(this.__vz==qx.fx.Base.EffectState.RUNNING){this.__vH=qx.fx.Transition.get(this.getTransition())(C)*this.__vE+this.getFrom();
...
}}},loop:function(c){if(c>=this.__vC){if(c>=this.__vD){this.end();
}var e=(c-this.__vC)/this.__vF;
var d=Math.round(e*this.__vG);
if(d>this.__vB){this.render(e);
this.__vB=d;
}}},cancel:function(){if(!this.getSync()){this.getQueue().remove(this);
}this.__vz=qx.fx.Base.EffectState.IDLE;
},resetState:function(){this.__vz=qx.fx.Base.EffectState.IDLE;
}},destruct:function(){this.__vA=this.__vz=null;
}});
})();
...
(function(){var e="__default",d="qx.fx.queue.Manager",c="__vK",b="singleton",a="object";
qx.Class.define(d,{extend:qx.core.Object,type:b,construct:function(){qx.core.Object.call(this);
this.__vK={};
},members:{__vK:null,getQueue:function(f){if(typeof (this.__vK[f])==a){return this.__vK[f];
}else{return this.__vK[f]=new qx.fx.queue.Queue;
...
this.__vL=[];
},properties:{limit:{init:Infinity,check:a}},members:{__vM:null,__vL:null,add:function(c){var d=new Date().getTime();
c._startOn+=d;
c._finishOn+=d;

if(this.__vL.length<this.getLimit()){this.__vL.push(c);
}else{c.resetState();
}
if(!this.__vM){this.__vM=qx.lang.Function.periodical(this.loop,15,this);
}},remove:function(e){qx.lang.Array.remove(this.__vL,e);

if(this.__vL.length==0){window.clearInterval(this.__vM);
delete this.__vM;
}},loop:function(){var f=new Date().getTime();

for(var i=0,g=this.__vL.length;i<g;i++){this.__vL[i]&&this.__vL[i].loop(f);
}}},destruct:function(){this.__vL=null;
}});
})();
(function(){var c="Number",b="static",a="qx.fx.Transition";
qx.Class.define(a,{type:b,statics:{get:function(i){return qx.fx.Transition[i]||false;
...
},pulse:function(k,l){l=(typeof (l)==c)?l:5;
return (Math.round((k%(1/l))*l)==0?Math.floor((k*l*2)-(k*l*2)):1-Math.floor((k*l*2)-(k*l*2)));
},spring:function(g){return 1-(Math.cos(g*4.5*Math.PI)*Math.exp(-g*6));
},none:function(j){return 0;
},full:function(p){return 1;
}}});
})();
(function(){var r='px',q="Boolean",p="Number",o="fontSize",n="Array",m="qx.fx.effect.core.Scale",l="12px",k='pt',j='em',i="position",f='%',h="100%",g="string",e="absolute";
qx.Class.define(m,{extend:qx.fx.Base,construct:function(B){qx.fx.Base.call(this,B);
this.__Lb=qx.fx.effect.core.Scale.originalStyle;
this.__Lc=qx.fx.effect.core.Scale.fontTypes;
},properties:{scaleX:{init:true,check:q},scaleY:{init:true,check:q},scaleContent:{init:true,check:q},scaleFromCenter:{init:true,check:q},scaleFrom:{init:100.0,check:p},scaleTo:{init:100,check:p},restoreAfterFinish:{init:false,check:q},alternateDimensions:{init:[],check:n}},statics:{originalStyle:{'top':null,'left':null,'width':null,'height':null,'fontSize':null},fontTypes:{'em':j,'px':r,'%':f,'pt':k}},members:{__Ld:null,__Le:null,__Lf:null,__Lg:null,__Lh:null,__Li:null,__Lj:null,__Lb:null,__Lc:null,setup:function(){qx.fx.Base.prototype.setup.call(this);
var w=this._getElement();
this.__Ld=qx.bom.element.Style.get(w,i);

for(var u in this.__Lb){this.__Lb[u]=w.style[u];
}this.__Le=qx.bom.element.Location.getTop(w);
this.__Lf=qx.bom.element.Location.getLeft(w);

try{var v=qx.bom.element.Style.get(w,o);
}catch(A){if(typeof (v)!=g){v=(qx.bom.client.Engine.MSHTML)?l:h;
}}
for(var s in this.__Lc){if(v.indexOf(s)>0){this.__Lg=parseFloat(v);
this.__Lh=s;
break;
}}this.__Li=(this.getScaleTo()-this.getScaleFrom())/100;
var t=this.getAlternateDimensions();

if(t.length==0){this.__Lj=[w.offsetWidth,w.offsetHeight];
}else{this.__Lj=t;
}},update:function(a){var c=this._getElement();
qx.fx.Base.prototype.update.call(this);
var b=(this.getScaleFrom()/100.0)+(this.__Li*a);

if(this.getScaleContent()&&this.__Lg){qx.bom.element.Style.set(c,o,this.__Lg*b+this.__Lh);
}this._setDimensions(this.__Lj[0]*b,this.__Lj[1]*b);
},finish:function(){qx.fx.Base.prototype.finish.call(this);
var z=this._getElement();

if(this.getRestoreAfterFinish()){for(var x in this.__Lb){var y=this.__Lb[x];
...
if(F){d.width=Math.round(C)+r;
}
if(G){d.height=Math.round(D)+r;
}
if(this.getScaleFromCenter()){var I=(C-this.__Lj[0])/2;
var J=(D-this.__Lj[1])/2;

if(this.__Ld==e){if(G){d.top=this.__Le-J+r;
}
if(F){d.left=this.__Lf-I+r;
}}else{if(G){d.top=-J+r;
}
if(F){d.left=-I+r;
}}}
for(var E in d){qx.bom.element.Style.set(H,E,d[E]);
}}},destruct:function(){this.__Lj=this.__Lb=this.__Lc=null;
}});
})();
(function(){var i="px",h="position",g="relative",f="Number",e="absolute",d="top",c="left",b='absolute',a="qx.fx.effect.core.Move";
qx.Class.define(a,{extend:qx.fx.Base,properties:{mode:{init:g,check:[g,e]},x:{init:0,check:f},y:{init:0,check:f}},members:{__KN:null,__KO:null,__KP:null,__KQ:null,__KR:null,setup:function(){var j=this._getElement();
qx.fx.Base.prototype.setup.call(this);

if(j.parentNode){this.__KP=qx.bom.element.Location.getLeft(j)-qx.bom.element.Location.getLeft(j.parentNode);
this.__KQ=qx.bom.element.Location.getTop(j)-qx.bom.element.Location.getTop(j.parentNode);
}else{this.__KP=qx.bom.element.Location.getLeft(j);
this.__KQ=qx.bom.element.Location.getTop(j);
}this.__KR=qx.bom.element.Style.get(j,"position");
qx.bom.element.Style.set(j,h,e);

if(this.getMode()==b){this.__KN=this.getX()-this.__KP;
this.__KO=this.getY()-this.__KQ;
}else{this.__KN=this.getX();
this.__KO=this.getY();
}},update:function(k){var m=this._getElement();
qx.fx.Base.prototype.update.call(this);
var l=Math.round(this.__KN*k+this.__KP);
var top=Math.round(this.__KO*k+this.__KQ);
qx.bom.element.Style.set(m,c,l+i);
qx.bom.element.Style.set(m,d,top+i);
},afterFinishInternal:function(){qx.bom.element.Style.set(this._getElement(),h,this.__KR);
}}});
})();
...
qx.Class.define(f,{extend:qx.fx.Base,properties:{startColor:{init:e,check:h},endColor:{init:c,check:h},restoreBackground:{init:true,check:j},keepBackgroundImage:{init:false,check:j}},members:{__Lr:null,__Ls:null,__Lt:null,__Lu:null,setup:function(){qx.fx.Base.prototype.setup.call(this);
var l=this._getElement();
this.__Lr={backgroundImage:qx.bom.element.Style.get(l,i),backgroundColor:qx.bom.element.Style.get(l,k)};

if(!this.getKeepBackgroundImage()){qx.bom.element.Style.set(l,i,g);
}this.__Ls=qx.util.ColorUtil.cssStringToRgb(this.getStartColor());
this.__Lt=qx.util.ColorUtil.cssStringToRgb(this.getEndColor());
this.__Lu=[this.__Lt[0]-this.__Ls[0],this.__Lt[1]-this.__Ls[1],this.__Lt[2]-this.__Ls[2]];
},update:function(m){qx.fx.Base.prototype.update.call(this);
var n=[this.__Ls[0]+Math.round(this.__Lu[0]*m),this.__Ls[1]+Math.round(this.__Lu[1]*m),this.__Ls[2]+Math.round(this.__Lu[2]*m)];
var o=d+qx.util.ColorUtil.rgbToHexString([n[0].toString(16),n[1].toString(16),n[2].toString(16)]);
qx.bom.element.Style.set(this._getElement(),k,o);
},finish:function(){qx.fx.Base.prototype.finish.call(this);

if(this.getRestoreBackground()){qx.lang.Function.delay(this._restore,1000,this);
}},_restore:function(){var b=this._getElement();

for(var a in this.__Lr){qx.bom.element.Style.set(b,a,this.__Lr[a]);
}}},destruct:function(){this.__Ls=this.__Lt=this.__Lu=null;
}});
})();
(function(){var e="horizontal",d="vertical",c="qx.fx.effect.combination.Shake",b="__KM",a="Number";
qx.Class.define(c,{extend:qx.fx.Base,construct:function(k){qx.fx.Base.call(this,k);
this.__KM=[new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k),new qx.fx.effect.core.Move(k)];
},properties:{direction:{init:e,check:[e,d]},duration:{init:0.5,refine:true},distance:{init:20,check:a}},members:{__KM:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var f=parseFloat(this.getDistance());
var g=parseFloat(this.getDuration())/10.0;

if(this.getDirection()==e){this.__KM[0].set({x:f,y:0,duration:g});
this.__KM[1].set({x:-f*2,y:0,duration:g*2});
this.__KM[2].set({x:f*2,y:0,duration:g*2});
this.__KM[3].set({x:-f*2,y:0,duration:g*2});
this.__KM[4].set({x:f*2,y:0,duration:g*2});
this.__KM[5].set({x:-f,y:0,duration:g*2});
}else if(this.getDirection()==d){this.__KM[0].set({y:f,x:0,duration:g});
this.__KM[1].set({y:-f*2,x:0,duration:g*2});
this.__KM[2].set({y:f*2,x:0,duration:g*2});
this.__KM[3].set({y:-f*2,x:0,duration:g*2});
this.__KM[4].set({y:f*2,x:0,duration:g*2});
this.__KM[5].set({y:-f,x:0,duration:g*2});
}var j=this.__KM;

for(var i=0,h=this.__KM.length;i<h;i++){this.__KM[i].id=i;

if(i<5){this.__KM[i].afterFinishInternal=function(){j[this.id+1].start();
};
}}this.__KM[0].start();
}},destruct:function(){this._disposeArray(b);
}});
})();
(function(){var t="linear",s="Number",r="reverse",q="Boolean",p="none",o="wobble",n="flicker",m="pulse",l="sinodial",k="easeOutQuad",d="spring",j="easeInQuad",g="full",c="Color",b="backgroundColor",f="#ffffff",e="backgroundImage",h="qx.fx.effect.combination.ColorFlow",a="__Lv",i="#ffffaa";
qx.Class.define(h,{extend:qx.fx.Base,construct:function(v){qx.fx.Base.call(this,v);
this.__Lv=[new qx.fx.effect.core.Highlight(v),new qx.fx.effect.core.Highlight(v)];
},properties:{startColor:{init:f,check:c},endColor:{init:i,check:c},forwardTransition:{init:t,check:[t,j,k,l,r,n,o,m,d,p,g]},backwardTransition:{init:t,check:[t,j,k,l,r,n,o,m,d,p,g]},forwardDuration:{init:1.0,check:s},backwardDuration:{init:1.0,check:s},delayBetween:{init:0.3,check:s},restoreBackground:{init:true,check:q},keepBackgroundImage:{init:false,check:q}},members:{__Lw:null,__Lv:null,start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var u=this._getElement();
this.setDuration(this.getForwardDuration()+this.getDelayBetween()+this.getBackwardDuration());
this.__Lw={backgroundImage:qx.bom.element.Style.get(u,e),backgroundColor:qx.bom.element.Style.get(u,b)};
this.__Lv[0].set({startColor:this.getStartColor(),endColor:this.getEndColor(),duration:this.getForwardDuration(),transition:this.getForwardTransition(),restoreBackground:false,keepBackgroundImage:this.getKeepBackgroundImage()});
this.__Lv[1].set({startColor:this.getEndColor(),endColor:this.getStartColor(),duration:this.getBackwardDuration(),transition:this.getBackwardTransition(),restoreBackground:this.getRestoreBackground(),keepBackgroundImage:this.getKeepBackgroundImage(),delay:this.getDelayBetween()});
var self=this;
this.__Lv[0].afterFinishInternal=function(){self.__Lv[1].start();
};
this.__Lv[0].start();
}},destruct:function(){this._disposeArray(a);
}});
})();
(function(){var d="opacity",c="qx.fx.effect.combination.Pulsate",b="__Lx",a="sinodial";
qx.Class.define(c,{extend:qx.fx.Base,construct:function(e){qx.fx.Base.call(this,e);
var f=this.getDuration()/6;
var g=0;
this.__Lx=[new qx.fx.effect.core.Fade(e),new qx.fx.effect.core.Fade(e),new qx.fx.effect.core.Fade(e),new qx.fx.effect.core.Fade(e),new qx.fx.effect.core.Fade(e),new qx.fx.effect.core.Fade(e)];

for(var i=0,l=this.__Lx.length;i<l;i++){this.__Lx[i].set({duration:f,to:((g%2)!=0)?1:0,from:((g%2)!=0)?0:1,transition:a,modifyDisplay:false});
g++;
}},properties:{duration:{init:2,refine:true}},members:{__Ly:null,__Lx:null,beforeSetup:function(){this.__Ly=qx.bom.element.Style.get(this._getElement(),d);
},start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var h=0;
var self=this;

for(var i=0,l=this.__Lx.length;i<l;i++){this.__Lx[i].id=h;

if(h<5){this.__Lx[i].afterFinishInternal=function(){self.__Lx[this.id+1].start();
};
}h++;
}this.__Lx[0].start();
},afterFinish:function(){qx.bom.element.Style.set(this._getElement(),d,this.__Ly);
}},destruct:function(){this._disposeArray(b);
}});
})();
(function(){var i="opacity",h="Boolean",g="none",f="qx.fx.effect.combination.Puff",e="_effects",d="__LB",c="__LA",b="__Lz",a="display";
qx.Class.define(f,{extend:qx.fx.Base,construct:function(l){qx.fx.Base.call(this,l);
this.__Lz=new qx.fx.effect.core.Scale(l);
this.__LA=new qx.fx.effect.core.Fade(l);
this.__LB=new qx.fx.effect.core.Parallel(this.__Lz,this.__LA);
},properties:{modifyDisplay:{init:true,check:h}},members:{__LA:null,__Lz:null,__LB:null,afterFinishInternal:function(){if(this.getModifyDisplay()){qx.bom.element.Style.set(this._getElement(),a,g);
}},start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var k=this._getElement();
var j={opacity:qx.bom.element.Style.get(k,i)};
this.__LA.afterFinishInternal=function(){var n=this._getElement();

for(var m in j){qx.bom.element.Style.set(n,m,j[m]);
}};
this.__Lz.set({scaleTo:200,sync:true,scaleFromCenter:true,scaleContent:true,restoreAfterFinish:true});
this.__LA.set({sync:true,to:0.0,modifyDisplay:false});
this.__LB.start();
}},destruct:function(){this._disposeArray(e);
this._disposeObjects(d,b,c);
}});
})();
(function(){var b="qx.fx.effect.core.Parallel",a="__Lk";
qx.Class.define(b,{extend:qx.fx.Base,construct:function(c){qx.fx.Base.call(this);
this.__Lk=arguments;
},members:{__Lk:null,finish:function(){qx.fx.Base.prototype.finish.call(this);
var d=this.__Lk;
...
(function(){var k="sinodial",j="none",i="overflow",h="flicker",g="center",f="pulse",e="spring",d="full",c="wobble",b="easeOutQuad",H="easeInQuad",G="reverse",F="linear",E="scroll",D="Boolean",C="px",B='bottom-left',A='top-left',z='bottom-right',y="bottom-right",r="opacity",s="bottom-left",p="visible",q="qx.fx.effect.combination.Shrink",n="top-left",o="top-right",l='top-right',m="__Lm",t='center',u="__Ll",w="hidden",v="__Ln",x="display";
qx.Class.define(q,{extend:qx.fx.Base,construct:function(a){qx.fx.Base.call(this,a);
this.__Ll=new qx.fx.effect.core.Move(a);
this.__Lm=new qx.fx.effect.core.Scale(a);
this.__Ln=new qx.fx.effect.core.Parallel(this.__Ll,this.__Lm);
},properties:{direction:{init:g,check:[n,o,s,y,g]},moveTransition:{init:k,check:[F,H,b,k,G,h,c,f,e,j,d]},scaleTransition:{init:k,check:[F,H,b,k,G,h,c,f,e,j,d]},modifyDisplay:{init:true,check:D}},members:{__Lo:null,__Ll:null,__Lm:null,__Ln:null,setup:function(){qx.fx.Base.prototype.setup.call(this);
qx.bom.element.Style.set(this._getElement(),i,w);
},afterFinishInternal:function(){qx.fx.Base.prototype.afterFinishInternal.call(this);
var N=this._getElement();
qx.bom.element.Style.set(N,i,p);
var M;

for(var L in this.__Lo){M=this.__Lo[L];

if(L!=i){M+=C;
}qx.bom.element.Style.set(N,L,M);
}
if(this.getModifyDisplay()){qx.bom.element.Style.set(N,x,j);
}},start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var K=this._getElement();
var I,J;
this.__Lo={top:qx.bom.element.Location.getTop(K,E),left:qx.bom.element.Location.getLeft(K,E),width:qx.bom.element.Dimension.getContentWidth(K),height:qx.bom.element.Dimension.getContentHeight(K),opacity:qx.bom.element.Style.get(K,r)};

switch(this.getDirection()){case A:I=J=0;
break;
case l:I=this.__Lo.width;
J=0;
break;
case B:I=0;
J=this.__Lo.height;
break;
case z:I=this.__Lo.width;
J=this.__Lo.height;
break;
case t:I=this.__Lo.width/2;
J=this.__Lo.height/2;
break;
}this.__Ll.set({x:I,y:J,sync:true,transition:this.getMoveTransition()});
this.__Lm.set({scaleTo:0,sync:true,transition:this.getScaleTransition(),restoreAfterFinish:true});
this.__Ln.start();
}},destruct:function(){this._disposeObjects(u,m,v);
}});
})();
(function(){var j="sinodial",i="px",h="flicker",g="0px",f="center",e="pulse",d="spring",c="full",b="wobble",a="easeOutQuad",J="easeInQuad",I="reverse",H="none",G="overflow",F="linear",E="height",D='bottom-left',C='top-left',B='bottom-right',A="bottom-right",q="qx.fx.effect.combination.Grow",r="bottom-left",o="visible",p="__La",m="width",n="top-left",k="top",l="top-right",s='top-right',t="__KX",v="block",u='center',x="__KY",w="hidden",z="left",y="display";
qx.Class.define(q,{extend:qx.fx.Base,construct:function(Q){qx.fx.Base.call(this,Q);
this.__KX=new qx.fx.effect.core.Move(Q);
this.__KY=new qx.fx.effect.core.Scale(Q);
this.__La=new qx.fx.effect.core.Parallel(this.__KX,this.__KY);
},properties:{direction:{init:f,check:[n,l,r,A,f]},scaleTransition:{init:j,check:[F,J,a,j,I,h,b,e,d,H,c]},moveTransition:{init:j,check:[F,J,a,j,I,h,b,e,d,H,c]}},members:{__KY:null,__KX:null,__La:null,setup:function(){qx.fx.Base.prototype.setup.call(this);
},start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var N=this._getElement();
...
var L,K;
var O,P;
var M={top:qx.bom.element.Location.getTop(N),left:qx.bom.element.Location.getLeft(N),width:qx.bom.element.Dimension.getContentWidth(N),height:qx.bom.element.Dimension.getContentHeight(N),overflow:o};
this.__KY.afterFinishInternal=function(){var S;
...
break;
}this.__KX.set({x:O,y:P,sync:true,transition:this.getMoveTransition()});
this.__KY.set({scaleTo:100,sync:true,scaleFrom:0,scaleFromCenter:false,transition:this.getScaleTransition(),alternateDimensions:[M.width,M.height]});
qx.bom.element.Style.set(N,k,(M.top+K)+i);
qx.bom.element.Style.set(N,z,(M.left+L)+i);
qx.bom.element.Style.set(N,E,g);
qx.bom.element.Style.set(N,m,g);
this.__La.start();
}},destruct:function(){this._disposeObjects(t,x,p);
}});
})();
(function(){var u="out",t="px",s="south",r="Number",q="north-west",p="west",o="in",n="north",m="south-east",l="south-west",e="east",k="north-east",h="display",c="__LC",b="__LE",g="top",f="Boolean",i="block",a="left",j="__LD",d="qx.fx.effect.combination.Drop";
qx.Class.define(d,{extend:qx.fx.Base,construct:function(A){qx.fx.Base.call(this,A);
this.__LC=new qx.fx.effect.core.Move(A);
this.__LD=new qx.fx.effect.core.Fade(A);
this.__LE=new qx.fx.effect.core.Parallel(this.__LC,this.__LD);
},properties:{direction:{init:s,check:[s,p,e,n,l,m,k,q]},xAmount:{init:100,check:r},yAmount:{init:100,check:r},mode:{init:u,check:[o,u]},modifyDisplay:{init:true,check:f}},members:{__LD:null,__LC:null,__LE:null,start:function(){qx.fx.Base.prototype.start.call(this);
var z=this._getElement();
...
this.__LD.afterFinishInternal=function(){for(var B in y){qx.bom.element.Style.set(z,B,y[B]+t);
}};
this.__LD.set({duration:0.5,sync:true,from:(this.getMode()==u)?1:0,to:(this.getMode()==u)?0:1,modifyDisplay:true});
this.__LE.start();
}},destruct:function(){this._disposeObjects(c,j,b);
}});
})();
(function(){var q="display",p="in",o="overflow",n="block",m="none",l="visible",k="0px",j="hidden",i="visiblity",h="Boolean",c="height",g="width",f="px",b="qx.fx.effect.combination.Fold",a="__LG",e="__LF",d="out";
qx.Class.define(b,{extend:qx.fx.Base,construct:function(v){qx.fx.Base.call(this,v);
},properties:{modifyDisplay:{init:true,check:h},mode:{init:p,check:[p,d]}},members:{__LF:null,__LG:null,__LH:null,afterFinish:function(){var r=this._getElement();

if((this.getModifyDisplay())&&(this.getMode()==p)){qx.bom.element.Style.set(r,q,n);
}},start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}var w=this._getElement();
var self=this;
this.__LF=new qx.fx.effect.core.Scale(w);
this.__LG=new qx.fx.effect.core.Scale(w);
this.__LF.afterFinishInternal=function(){self.__LG.start();
};
this.__LG.afterFinishInternal=function(){self._cleanUp();
};
this.__LH=this._getStyle();
qx.bom.element.Style.set(w,o,j);

if(this.getMode()==p){this.__LF.set({scaleTo:5,scaleContent:false,scaleX:false,duration:this.getDuration()/2,scaleFrom:100,scaleFromCenter:true,alternateDimensions:[]});
this.__LG.set({scaleTo:5,scaleContent:false,scaleY:false,duration:this.getDuration()/2,scaleFrom:100,scaleFromCenter:true,alternateDimensions:[]});
}else{this.__LF.set({scaleTo:100,scaleContent:false,scaleY:false,duration:this.getDuration()/2,scaleFrom:0,scaleFromCenter:true,alternateDimensions:[this.__LH.width,this.__LH.height]});
this.__LG.set({scaleTo:100,scaleContent:false,scaleX:false,duration:this.getDuration()/2,scaleFrom:0,scaleFromCenter:false,alternateDimensions:[this.__LH.width,this.__LH.height]});
qx.bom.element.Style.set(w,q,n);
qx.bom.element.Style.set(w,c,k);
qx.bom.element.Style.set(w,g,k);
}this.__LF.start();
},_cleanUp:function(){var t;
var u=this._getElement();

if((this.getMode()==p)&&(this.getModifyDisplay())){qx.bom.element.Style.set(u,q,m);
}
for(var s in this.__LH){t=this.__LH[s];

if(s!=o){t+=f;
}qx.bom.element.Style.set(u,s,t);
}qx.bom.element.Style.set(u,o,l);
...
(function(){var j="off",i="overflow",h="Boolean",g="qx.fx.effect.combination.Switch",f="none",e="__LJ",d="flicker",c="__LI",b="hidden",a="display";
qx.Class.define(g,{extend:qx.fx.Base,construct:function(k){qx.fx.Base.call(this,k);
this.setTransition(d);
var l=this.__LI=new qx.fx.effect.core.Scale(k);
this.__LI.beforeSetup=function(){qx.bom.element.Style.set(k,i,b);
};
this.__LJ=new qx.fx.effect.core.Fade(k);
this.__LJ.afterFinishInternal=function(){l.start();
};
},properties:{duration:{init:0.5,refine:true},from:{init:0.0,refine:true},to:{init:1.0,refine:true},modifyDisplay:{init:true,check:h},mode:{init:j,check:[j]}},members:{__LI:null,__LJ:null,setup:function(){qx.fx.Base.prototype.setup.call(this);
var n=this._getElement();
var m=qx.bom.element.Style.get(n,i);
this.__LI.afterFinishInternal=function(){qx.bom.element.Style.set(n,i,m);
};
},afterFinish:function(){if(this.getModifyDisplay()&&(this.getMode()==j)){qx.bom.element.Style.set(this._getElement(),a,f);
}},start:function(){if(!qx.fx.Base.prototype.start.call(this)){return;
}
if(this.getMode()==j){this.__LI.set({scaleTo:1.0,duration:this.getDuration()/2,scaleFromCenter:true,scaleX:false,scaleContent:false,restoreAfterFinish:true});
this.__LJ.set({duration:this.getDuration()/2,from:this.getFrom(),to:1});
}else{}this.__LJ.start();
},_applyDuration:function(o,p){this.__LI.setDuration(o/2);
this.__LJ.setDuration(o/2);
