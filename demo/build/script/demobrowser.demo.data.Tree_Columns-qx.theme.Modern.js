...
var O=new qx.ui.container.Composite(new qx.ui.layout.Basic());
O.setAllowGrowX(false);
O.setAllowStretchX(false);
P.add(O);
this.getRoot().add(P,{edge:0});
var Q=new qx.ui.tree.Tree().set({width:600,height:500});
O.add(Q,{left:20,top:48});
var S={label:w,children:[{label:H,children:[{label:q},{label:l,children:[{label:a},{label:K}]},{label:B},{label:d}]},{label:I,children:[{label:C},{label:r},{label:d},{label:t},{label:z},{label:y},{label:F},{label:p,children:[]},{label:m}]}]};

for(var i=0;i<50;i++){S.children[1].children[7].children[i]={label:A+i};
}this.extendData(S);
var R=qx.data.marshal.Json.createModel(S);
var T=new qx.data.controller.Tree(null,Q,s,v);
T.setDelegate(this);
T.setModel(R);
},extendData:function(V){V.date=u+Math.round(Math.random()*30+1)+o;
V.size=Math.round(Math.random()*100)+x;
V.light=Math.floor(Math.random()*4)==0;
V.mode=D;
V.checked=Math.random()>=0.5;

if(V.children){for(var i=0;i<V.children.length;i++){this.extendData(V.children[i]);
}}},bindItem:function(L,M,N){L.bindDefaultProperties(M,N);
L.bindProperty(j,k,null,M.getUserData(j),N);
L.bindProperty(b,k,null,M.getUserData(e),N);
L.bindPropertyReverse(b,k,null,M.getUserData(e),N);
L.bindProperty(f,k,null,M.getUserData(f),N);
L.bindProperty(g,k,null,M.getUserData(g),N);
L.bindProperty(h,E,{converter:function(U){return U?n:c;
}},M.getUserData(h),N);
