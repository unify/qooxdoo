...
var ba=new qx.ui.layout.HBox(5);
this.topContainer=new qx.ui.container.Composite(ba);
this.runTest(x);
this.runTest(a);
this.runTest(A);
this.runTest(B);
...
},testStringCell:function(){var U={value:[f,h,j,null],states:[{},{selected:1}]};
var T=new qx.ui.virtual.cell.String;
qx.util.Permutation.permute(U,function(X){var Y=this.__Qj(T,X.value,X.states);
...
qx.util.Permutation.permute(bh,function(bv){if(bv.format){bi.setNumberFormat(bv.format);
}else{bi.resetNumberFormat();
}var bw=this.__Qj(bi,bv.value,bv.states);
this.container.add(bw);
},this);
},testDateCell:function(){var E={value:[new Date(),null],format:[null,new qx.util.format.DateFormat(qx.locale.Date.getDateFormat(p))],states:[{},{selected:1}]};
var F=new qx.ui.virtual.cell.Date();
qx.util.Permutation.permute(E,function(bn){if(bn.format){F.setDateFormat(bn.format);
}else{F.resetDateFormat();
}var bo=this.__Qj(F,bn.value,bn.states);
this.container.add(bo);
},this);
},testHtmlCell:function(){var bf={value:[f,h,j,null],states:[{},{selected:1}]};
var bg=new qx.ui.virtual.cell.Html;
qx.util.Permutation.permute(bf,function(bb){var bc=this.__Qj(bg,bb.value,bb.states);
this.container.add(bc);
},this);
},testImageCell:function(){var R={value:[{url:k,tooltip:s},{url:o,tooltip:q},null],states:[{},{selected:1}]};
var S=new qx.ui.virtual.cell.Image;
qx.util.Permutation.permute(R,function(bl){var bm=this.__Qj(S,bl.value,bl.states);
...
qx.util.Permutation.permute(V,function(bd){var be=this.__Qj(W,bd.value,bd.states);
this.container.add(be);
},this);
},testBooleanCellCustomImage:function(){var bj={value:[true,false,null],states:[{},{selected:1}]};
var bk=new qx.ui.virtual.cell.Boolean;
bk.setIconFalse(n);
bk.setIconTrue(l);
qx.util.Permutation.permute(bj,function(G){var H=this.__Qj(bk,G.value,G.states);
this.container.add(H);
},this);
},__Qj:function(I,J,K){var O=100;
var M=30;
var Q=new qx.ui.embed.Html().set({width:O,height:M,decorator:K.selected?null:m});
var L=I.getCellProperties(J,K);
var N=L.insets;
var P=[b,t,this._getCellSizeStyle(O,M,N[0],N[1]),L.style,i,D,L.classes,i,L.attributes,z,L.content,c].join(j);
Q.setHtml(P);
return Q;
},_getCellSizeStyle:function(bq,br,bs,bt){var bu=j;

if(qx.bom.client.Feature.CONTENT_BOX){bq-=bs;
br-=bt;
}bu+=C+bq+g;
bu+=y+br+g;
return bu;
...
},members:{__Qk:16,__Ql:16,_aliasManager:null,__Qm:function(t){var w=qx.util.ResourceManager.getInstance();
...
},__Qn:function(H){if(typeof (H)==h){H={url:H};
}var I=this._aliasManager.resolve(H.url||null);
var J;

if(H.width&&H.height){J={width:H.width,height:H.height};
}else{J=this.__Qm(I);
}return {width:J.width,height:J.height,url:I,tooltip:H.tooltip};
},_identifyImage:function(K){throw new Error("_identifyImage is abstract");
},getContent:function(y,z){if(y===null){return s;
}var content=s;
var B=this.__Qn(this._identifyImage(y));
var F=qx.bom.client.Engine.GECKO&&qx.bom.client.Engine.VERSION<1.9;
var C=B.tooltip?m+B.tooltip+o:s;
var A={width:B.width+r,height:B.height+r,display:F?a:i,verticalAlign:b,position:c};
var D=qx.bom.element.Decoration.getTagName(p,B.url);
var E=qx.bom.element.Decoration.getAttributes(B.url,p,A);
var G=qx.bom.element.Style.compile(E.style);

if(D===d){content=f+E.src+n+G+q;
content+=C+g;
}else{content=l+G+q;
content+=C+k;
}return content;
...
this.__Qo=qx.util.AliasManager.getInstance();
},properties:{appearance:{refine:true,init:a},iconTrue:{check:e,themeable:true,apply:b},iconFalse:{check:e,themeable:true,apply:d}},members:{__Qp:null,__Qq:null,__Qo:null,_applyIconTrue:function(h){this.__Qp=this.__Qo.resolve(h);
},_applyIconFalse:function(f){this.__Qq=this.__Qo.resolve(f);
},_identifyImage:function(g){return g==true?this.__Qp:this.__Qq;
}}});
})();
