...
},members:{__OD:null,__OC:null,_js_in_array:function(C,D){var E=D.toString();

if(E==a){return false;
}var G=new RegExp(C,p);
var F=G.test(D);
return F;
},addBetweenFilter:function(X,Y,ba,bb){if(this._js_in_array(X,this.betweenAllowed)&&bb!=null){if(Y!=null&&ba!=null){var bc=new Array(X,Y,ba,bb);
}}
if(bc!=null){this.Filters.push(bc);
}else{throw new Error("Filter not recognized or value1/value2 is null!");
}},addNumericFilter:function(H,I,J){var K=null;

if(this._js_in_array(H,this.numericAllowed)&&J!=null){if(I!=null){K=[H,I,J];
}}
...
}},addNotRegex:function(x,y,z){var B;

if(z){B=h;
}else{B=p;
}
if(x!=null&&y!=null){var A=new Array(m,x,y,B);
}
...
switch(this.Filters[i][0]){case o:if(L==this.Filters[i][1]){P=true;
}break;
case e:if(L!=this.Filters[i][1]){P=true;
}break;
case n:if(L>this.Filters[i][1]){P=true;
}break;
case k:if(L<this.Filters[i][1]){P=true;
}break;
case f:if(L>=this.Filters[i][1]){P=true;
}break;
case d:if(L<=this.Filters[i][1]){P=true;
}break;
}}else if(this._js_in_array(this.Filters[i][0],this.betweenAllowed)&&P==false){L=this.getValueById(this.Filters[i][3],R);

switch(this.Filters[i][0]){case l:if(L>=this.Filters[i][1]&&L<=this.Filters[i][2]){P=true;
}break;
case g:if(L<this.Filters[i][1]&&L>this.Filters[i][2]){P=true;
}break;
}}else if(this.Filters[i][0]==j&&P==false){L=this.getValueById(this.Filters[i][2],R);
var N=new RegExp(this.Filters[i][1],this.Filters[i][3]);
P=N.test(L);
}else if(this.Filters[i][0]==m&&P==false){L=this.getValueById(this.Filters[i][2],R);
var N=new RegExp(this.Filters[i][1],this.Filters[i][3]);
P=!N.test(L);
...
},hideRows:function(S,T,dispatchEvent){var V=this.getData();
dispatchEvent=(dispatchEvent!=null?dispatchEvent:true);

if(!this.__OC){this.__OD=V.slice(0);
this.__OC=true;
}
if(T==null||T<1){T=1;
}
for(var U=S;U<(V.length-T);U++){V[U]=V[U+T];
}this.removeRows(U,T);
if(dispatchEvent){var W={firstRow:0,lastRow:V.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(c,W);
}},resetHiddenRows:function(){if(!this.__OD){return ;
}this.Filters=[];
this.setData(qx.lang.Array.clone(this.__OD));
},setData:function(v,w){this.__OD=qx.lang.Array.clone(v);
this.Filters=[];
qx.ui.table.model.Simple.prototype.setData.call(this,v,w);
}},destruct:function(){this.__OD=this.numericAllowed=this.betweenAllowed=this.Filters=null;
}});
})();
