...
var M=0;
var I=function(a){var d=[];
var c=new Date().getTime();
var e=400*24*60*60*1000;

for(var f=0;f<a;f++){var b=new Date(c+Math.random()*e-e/2);
d.push({renderer:x,location:n,data:[M++,Math.random()*10000,b]});
}return d;
};
var L=new qx.ui.progressive.renderer.table.Widths(3);
L.setWidth(0,100);
L.setWidth(1,r);
L.setWidth(2,300);
var F=[j,t,l];
var H=new qx.ui.progressive.headfoot.TableHeading(L,F);
var E=new qx.ui.progressive.headfoot.Progress(L,F);
var O=new qx.ui.progressive.structure.Default(H,E);
var D=new qx.ui.progressive.Progressive(O);
var N=new qx.ui.basic.Atom(h);
N.setTextColor(s);
N.setHeight(16);
N.setRich(true);
D.add(N);
...
K.addElements(J);
D.setDataModel(K);
var G=new qx.ui.progressive.renderer.table.Row(L);
D.addRenderer(x,G);
G.addRenderer(2,new demobrowser.demo.progressive.DateCellRenderer());
this.getRoot().add(D,{edge:50});
D.render();
}}});
qx.Class.define(g,{extend:qx.ui.progressive.renderer.table.cell.Default,statics:{onmouseover:function(A){A.__NV=qx.bom.element.Style.get(A,"color");
A.style.color=w;
A.parentNode.style.height=32;
A.__NW=A.innerHTML;
var C=Date.parse(A.attributes.getNamedItem(v).nodeValue);
var B=new Date(C);
A.innerHTML=B.toUTCString()+o+B.toString();
},onmouseout:function(P){P.style.color=P.__NV;
P.parentNode.style.height=16;
P.innerHTML=P.__NW;
...
z.push(q+y.cellData.toUTCString()+p);
z.push(k,m);
return z.join(u);
}}});
})();
