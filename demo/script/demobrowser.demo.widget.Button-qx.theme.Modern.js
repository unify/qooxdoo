...
u.setPadding(20);
this.getRoot().add(u,{left:0,top:0});
this.addNormalButtons(u);
this.addToggleButton(u);
this.addRepeatButton(u);
this.addHoverButton(u);
},addNormalButtons:function(v){var w=new qx.ui.form.Button(l,f);
v.add(w);
var x=new qx.ui.form.Button(i,d);
x.setEnabled(false);
v.add(x);
},addToggleButton:function(D){var E=new qx.ui.form.ToggleButton(a,g);
E.focus();
D.add(E);
E.addListener(b,function(e){this.debug("Checked: "+e.getData());
},this);
},addRepeatButton:function(z){var A=new qx.ui.form.RepeatButton(null,j);
z.add(A);
var B=new qx.ui.basic.Label(p);
B.setDecorator(n);
B.setPadding(2,4);
B.setBackgroundColor(o);
z.add(B);
A.addListener(m,function(){var C=parseInt(B.getValue())+1;
B.setValue(C.toString());
});
},addHoverButton:function(q){var r=new qx.ui.form.HoverButton(c).set({padding:10,backgroundColor:k});
q.add(r);
...
q.add(s);
r.addListener(m,function(){var y=parseInt(s.getValue())+1;
s.setValue(y.toString());
});
}}});
})();
