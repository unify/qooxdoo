...
},createTable:function(){var bM=[[J,k,x,{'required':true}],[d,K,S,{'type':d,'required':true}],[L,A,T,{'options':[U,N,G],'editable':true}],[Q,v,a,{'options':[[n,null,a],[D,null,q],[w,null,E]]}],[e,z,y,{'type':e}],[l,O,M,{'regExp':W,'failMsg':Y}],[I,t,H,{'validationFunc':function(bN){if(bN.search(/^http:/)!=-1){window.open(bN);
}return bN;
}}],[V,m,true,{'type':j}]];
var bL=function(ba){var bg=ba.table;
var bd=bg.getTableModel();
var be=bd.getRowData(ba.row);
var bh=be[3];
var bf;

for(var bc in bh){switch(bc){case f:switch(bh[b]){case j:return new qx.ui.table.cellrenderer.Boolean;
case g:return new qx.ui.table.cellrenderer.Password;
}break;
case h:var bf=new qx.ui.table.cellrenderer.Replace;
var bb={};
bh[c].forEach(function(bi){if(bi instanceof Array){bb[bi[0]]=bi[2];
}});
bf.setReplaceMap(bb);
...
var bI=function(bj){var bo=bj.table;
...
break;
case s:bl.setValidationFunction(function(bu,bv){var bw=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*\.(\w{2}|(com|net|org|edu|int|mil|gov|arpa|biz|aero|name|coop|info|pro|museum))$/;

if(bw.test(bu)){return bu;
}alert("You did not enter a valid email address");
return bv;
});
break;
}break;
case r:bl.setValidationFunction(function(bB,bC){var bD=new RegExp(bq[X]);
...
break;
case o:bp=function(bs,bt){if(!bs){alert("You need to supply a value here");
return bt;
}return bs;
};
break;
...
bF.setColumns([i,i,p]);
var bE={tableColumnModel:function(br){return new qx.ui.table.columnmodel.Resize(br);
}};
...
bG.getTableModel().addListener(u,function(event){if(!(event instanceof qx.event.type.Data)){return;
...
}},properties:{cellEditorFactoryFunction:{check:b,nullable:true,init:null}},members:{__Ov:null,createCellEditor:function(c){var d=this.getCellEditorFactoryFunction();
{};
this.__Ov=d(c);
var e=this.__Ov.createCellEditor(c);
return e;
},getCellEditorValue:function(g){var h=this.getCellEditorFactoryFunction();
{};
var i=this.__Ov.getCellEditorValue(g);
return i;
