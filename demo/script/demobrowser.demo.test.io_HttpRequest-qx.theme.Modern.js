...
(function(){var H="Boolean",G="application/json",F="qx.event.type.Event",E="String",D="application/xml",C="text/html",B="application/x-www-form-urlencoded",A="Last-Modified",z="GET",y="qx.event.type.Data",bd="text/plain",bc="text/javascript",bb="POST",ba="qx.io.HttpRequest",Y="Integer",X="HEAD",W="error",V="change",U="Accept",T="load",O="OPTIONS",P="abort",M="",N="PUT",K="If-Modified-Since",L="DELETE",I="no-cache",J="Cache-Control",Q="Content-Type",R="timeout",S="Thu, 01 Jan 1970 00:00:00 GMT";
qx.Class.define(ba,{extend:qx.core.Object,construct:function(x){qx.core.Object.call(this);
this.__tG={};
if(x!=null){this.setUrl(x);
}},statics:{__tH:{},sync:function(h,i){this.__tH[h]=i;
},clear:function(b){delete this.__tH[b];
}},events:{"change":y,"timeout":y,"load":F,"error":F,"abort":F},properties:{url:{check:E,init:M},method:{check:[z,bb,N,X,L,O],init:z},async:{check:H,init:true},data:{nullable:true},requestType:{check:[B,G,D,bd,bc,C],init:B},responseType:{check:[G,D,bd,bc,C],init:G},username:{check:E,nullable:true},password:{check:E,nullable:true},timeout:{check:Y,nullable:false,init:10000},refresh:{check:H,init:false},cache:{check:H,init:false}},members:{__tI:null,__tG:null,setRequestHeader:function(o,p){{};
this.__tG[o]=p;
},removeRequestHeader:function(j){delete this.__tG[j];
},getRequestHeader:function(c){var d=this.__tG[c];

if(d===undefined){d=null;
}return d;
},getResponseText:function(){var be=this.__tI;

if(be){return be.responseText;
}},getResponseXml:function(){var bg=this.__tI;

if(bg){return bg.responseXML;
}},getResponseHeader:function(q){var r=this.__tI;

if(r){return r.getResponseHeader(q);
}},getAllResponseHeaders:function(){var g=this.__tI;

if(g){return g.getAllResponseHeaders();
}},getDuration:function(){var bf=this.__tI;
return bf?bf.getDuration():null;
},isModified:function(){var e=this.__tI;

if(!e){return null;
}return !(e.status===304||e.getResponseHeader(A)===qx.io.HttpRequest.__tH[this.getUrl()]);
},isSuccessful:function(){var f=this.__tI;
return !f||f.isSuccessful();
},getStatusCode:function(){var n=this.__tI;

if(n){return n.status;
}},getStatusText:function(){var m=this.__tI;

if(m){return m.statusText;
}},getReadyState:function(){var a=this.__tI;
return a?a.readyState:null;
},send:function(){if(this.__tI){if(this.getReadyState()!==4){throw new Error("Request is still pending at ready state: "+this.getReadyState());
}this.__tI.dispose();
}var u=this.__tI=new qx.bom.Request;
u.onreadystatechange=qx.lang.Function.bind(this.__tJ,this);
u.ontimeout=qx.lang.Function.bind(this.__tK,this);
u.onload=qx.lang.Function.bind(this.__tL,this);
u.onerror=qx.lang.Function.bind(this.__tM,this);
u.onabort=qx.lang.Function.bind(this.__tN,this);
var s=this.getUrl();
u.timeout=this.getTimeout();
u.open(this.getMethod(),s,this.getAsync(),this.getUsername(),this.getPassword());
if(!this.getCache()){u.setRequestHeader(J,I);
}if(this.getRefresh()){var v=qx.io.HttpRequest.__tH[s]||S;
u.setRequestHeader(K,v);
}if(this.getMethod()===bb){u.setRequestHeader(Q,this.getRequestType());
}u.setRequestHeader(U,this.getResponseType());
var t=this.__tG;

for(var w in t){u.setRequestHeader(w,t[w]);
}u.send(this.getData());
},abort:function(){if(this.__tI){this.__tI.abort();
}else{}},__tJ:function(){this.fireDataEvent(V,this.getReadyState());
},__tK:function(){this.fireDataEvent(R,this.getDuration());
},__tL:function(){if(this.getRefresh()&&this.getReadyState()==4&&this.isSuccessful()){var l=this.getResponseHeader(A);
var k=this.getUrl();
}this.fireEvent(T);
if(l){qx.io.HttpRequest.__tH[k]=l;
}},__tM:function(){this.fireEvent(W);
},__tN:function(){this.fireEvent(P);
}},destruct:function(){if(this.__tI){this.__tI.dispose();
this.__tI=null;
}}});
})();
(function(){var z="qx.client",y="gecko",x="Content-Type",w="",v="mshtml",u="application/xml",t="qx.bom.Request",s="file:",r="parsererror";
qx.Class.define(t,{extend:qx.core.Object,construct:function(){this.__tO={};
this.__tP=this.__ub();
},statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},members:{__tQ:null,__tR:null,__tP:null,__tS:null,__tT:null,__tU:null,__tV:null,__tO:null,readyState:0,responseText:w,responseXML:null,status:0,statusText:w,timeout:0,onreadystatechange:function(){},ontimeout:function(){},onload:function(){},onerror:function(){},onabort:function(){},open:function(c,d,e,f,g){if(e==null){e=true;
}this.__tQ=e;
this.__tR=qx.lang.Function.bind(this.__tX,this);
var h=this.__tP;
h.onreadystatechange=this.__tR;
if(arguments.length>4){h.open(c,d,e,f,g);
}else if(arguments.length>3){h.open(c,d,e,f);
}else{h.open(c,d,e);
}if(qx.core.Variant.isSet(z,y)){if(!e){this.readyState=qx.bom.Request.OPENED;
this.__ud();
}}},send:function(o){var p=this.__tO;
if(o&&o.nodeType){o=window.XMLSerializer?new XMLSerializer().serializeToString(o):o.xml;

if(!p[x]){p[x]=u;
}}for(var q in p){this.__tP.setRequestHeader(q,p[q]);
}this.__tW=(new Date).valueOf();
if(this.timeout!=null&&this.timeout>0){this.__tS=qx.lang.Function.bind(this.__tY,this);
this.__tT=window.setTimeout(this.__tS,this.timeout);
}this.__tP.send(o);
if(qx.core.Variant.isSet(z,y)){if(!this.__tQ){this.readyState=qx.bom.Request.OPENED;
this.__uf(this);
...
},abort:function(){if(this.__tT){window.clearTimeout(this.__tT);
}this.__ua();
this.onabort();
this.dispose();
},getDuration:function(){return this.__uc;
},__tX:qx.event.GlobalError.observeMethod(function(){if(qx.core.Variant.isSet(z,y)){if(!this.__tQ){return;
}}this.readyState=this.__tP.readyState;
this.__uf();
if(this.__tU){this.readyState=qx.bom.Request.UNSENT;
return ;
}if(this.readyState==qx.bom.Request.DONE&&this.__tT){window.clearTimeout(this.__tT);
}this.__ud();
if(this.readyState==qx.bom.Request.DONE){this.dispose();
}}),__tY:qx.event.GlobalError.observeMethod(function(){this.__ua();
this.ontimeout();
this.dispose();
}),getAllResponseHeaders:function(){try{return this.__tP.getAllResponseHeaders();
}catch(k){return null;
}},getResponseHeader:function(m){try{return this.__tP.getResponseHeader(m);
}catch(D){return null;
}},setRequestHeader:function(a,b){if(b==null){delete this.__tO[a];
}else{this.__tO[a]=b;
}},removeRequestHeader:function(A,B){delete this.__tO[A];
},getRequestHeader:function(l){return this.__tO[l]||null;
},__ua:function(){this.__uf();
if(this.readyState>qx.bom.Request.UNSENT){this.__tU=true;
}this.__tP.abort();
},__ub:qx.core.Variant.select(z,{"default":function(){return new XMLHttpRequest;
},"mshtml":function(){if(window.ActiveXObject&&qx.xml.Document.XMLHTTP){return new ActiveXObject(qx.xml.Document.XMLHTTP);
}
if(window.XMLHttpRequest){return new XMLHttpRequest;
}}}),__tW:null,__uc:null,__ud:function(){var n=this.readyState;
if(this.__tV===n){return;
}this.onreadystatechange();
this.__tV=n;
if(n===4){this.__uc=(new Date).valueOf()-this.__tW;

if(this.isSuccessful()){this.onload();
}else{this.onerror();
}}},__ue:function(){var E=this.__tP.responseXML;

if(qx.core.Variant.isSet(z,v)){if(E&&!E.documentElement&&this.__tP.getResponseHeader(x).match(/[^\/]+\/[^\+]+\+xml/)){E=new ActiveXObject(qx.xml.Document.DOMDOC);
E.loadXML(this.__tP.responseText);
}if(E&&E.parseError!=0){return null;
}}else if(!E.documentElement||E.documentElement.tagName==r){return null;
}return E;
},__uf:function(){var F=this.__tP;

try{this.responseText=F.responseText;
}catch(j){}
try{this.responseXML=this.__ue();
}catch(G){}
try{this.status=F.status;
}catch(C){}
try{this.statusText=F.statusText;
}catch(i){}if(qx.core.Variant.isSet(z,v)){if(this.status===1223){this.status=204;
}}if(!this.status&&location.protocol===s){this.status=204;
}}},destruct:function(){if(this.__tT){window.clearTimeout(this.__tT);
this.__tT=null;
}if(this.__tP){this.__tP.onreadystatechange=qx.lang.Function.empty;
this.__tP=null;
