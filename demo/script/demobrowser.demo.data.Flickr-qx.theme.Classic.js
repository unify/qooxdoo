...
(function(){var l="qooxdoo",k="/",j=".static.flickr.com/",i="http://api.flickr.com/services/rest/?tags=",h="",g="_",f="http://farm",d="Enter",c="keydown",b="model.photos.photo",H=".jpg",G="loaded",F="execute",E="source",D="selection[0]",C="_s.jpg",B="<span style='font-size: 20px'>Flickr</span>",A="failed",z="loading",y="value",s="demobrowser.demo.data.store.Flickr",t="demobrowser.demo.data.Flickr",q="title",r="icon",o="state",p="completed",m="Search",n="receiving",u="horizontal",v="model",x="&method=flickr.photos.search&api_key=63a8042eead205f7e0040f488c02afd9&format=json",w="id";
...
var bb=new demobrowser.demo.data.store.Flickr(l);
var S=new qx.ui.form.TextField(l);
this.getRoot().add(S,{left:30,top:50});
var T=new qx.ui.form.Button(m);
this.getRoot().add(T,{left:140,top:49});
T.addListener(F,function(){bb.searchForTag(S.getValue());
},this);
S.addListener(c,function(e){if(e.getKeyIdentifier()==d){bb.searchForTag(S.getValue());
}},this);
var status=new qx.ui.basic.Label(z);
...
W.setHeight(110);
W.setOrientation(u);
this.getRoot().add(W,{left:30,right:30,top:80});
var Y=new qx.data.controller.List(null,W);
Y.setLabelPath(q);
Y.setDelegate({configureItem:function(I){I.setShow(r);
}});
var U={converter:function(K,L){return (f+L.getFarm()+j+L.getServer()+k+K+g+L.getSecret()+C);
}};
Y.setIconOptions(U);
Y.setIconPath(w);
bb.bind(b,Y,v);
var V=new qx.ui.basic.Image();
this.getRoot().add(V,{left:30,top:200});
var ba={converter:function(R){if(R){return (f+R.getFarm()+j+R.getServer()+k+R.getId()+g+R.getSecret()+H);
}return h;
}};
Y.bind(D,V,E,ba);
...
qx.Class.define(s,{extend:qx.data.store.Json,statics:{RESULT:null},construct:function(M){jsonFlickrApi=function(bc){demobrowser.demo.data.store.Flickr.RESULT=bc;
};
var N=i+M;
qx.data.store.Json.call(this,N);
},members:{searchForTag:function(Q){if(Q!=h){this.setUrl(i+Q);
}},_createRequest:function(O){var P=new qx.io.ScriptLoader();
this.setState(n);
O+=x;
P.load(O,function(J){this.__Mo();
},this);
},__Mo:function(){this.setState(p);
var a=demobrowser.demo.data.store.Flickr.RESULT;

if(a==undefined){this.setState(A);
return;
