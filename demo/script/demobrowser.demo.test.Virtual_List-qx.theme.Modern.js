...
bN.addListener(l,function(e){if(e.getData()){var bT=bP.getSelectionMode();

if(bT==m||bT==j){this.debug("Quick selection is only available for the modes multi or additive");
}}bP.setQuickSelection(e.getData());
});
bG.addListener(T,function(e){var bC=e.getData()[0].getUserData(o);
bP.setSelectionMode(bC);

if(bC==k||bC==n){bK.setEnabled(false);
bN.setEnabled(true);
}else if(bC==m||bC==q){bK.setEnabled(true);
bN.setEnabled(false);
}});
...
(function(){var S="selected",R="qx.event.type.Data",Q="Boolean",P="changeSelection",O="row",N="single",M="changeHeight",L=",",K="one",J="_applySpacing",bi="Integer",bh="_applyDragSelection",bg="String",bf="resize",be="demobrowser.demo.virtual.list.List",bd="list",bc="multi",bb="_applyQuickSelection",ba="_applyItemHeight",Y="_applySelectionMode",W="additive",X="changeName",U="Number",V="addItem",T="removeItem";
qx.Class.define(be,{extend:qx.ui.virtual.core.Scroller,implement:[qx.ui.virtual.core.IWidgetCellProvider],construct:function(){qx.ui.virtual.core.Scroller.call(this,0,1,this.getItemHeight(),10);
this.__Pn=new qx.ui.virtual.layer.WidgetCell(this);
this.getPane().addLayer(this.__Pn);
this.getPane().addListener(bf,this._onResize,this);
this.__Po=[];
this.__Pp=[];
this.__Pq=new qx.ui.virtual.selection.Row(this.getPane());
this.__Pq.addListener(P,this._onChangeSelection,this);
this.__Pq.attachMouseEvents();
this.__Pq.attachKeyEvents(this);
this.__Pq.attachListEvents(this);
...
},events:{addItem:R,removeItem:R,changeValue:R,changeSelection:R},properties:{appearance:{refine:true,init:bd},focusable:{refine:true,init:true},spacing:{check:bi,init:0,apply:J,themeable:true},enableInlineFind:{check:Q,init:true},name:{check:bg,nullable:true,event:X},itemHeight:{init:24,themeable:true,check:U,apply:ba},selectionMode:{check:[N,bc,W,K],init:N,apply:Y},dragSelection:{check:Q,init:false,apply:bh},quickSelection:{check:Q,init:false,apply:bb}},members:{__Po:null,__Pp:null,__Pq:null,__Pn:null,syncWidget:function(){this.update();
},update:function(){var C=this.getPane().getRowConfig();
C.setItemCount(this.__Po.length);
C.resetItemSizes();

for(var i=0;i<this.__Po.length;i++){var B=this.__Po[i].getHeight();

if(B!==null){C.setItemSize(i,B);
}}},updateSelection:function(){var h=this.__Pn.getChildren();

for(var i=0;i<h.length;i++){var k=h[i];
var m=k.getUserData(O);

if(this.__Pq.isItemSelected(m)){k.addState(S);
}else{k.removeState(S);
...
this.fireDataEvent(P,this.__Pu(e.getData()));
},getCellWidget:function(bo,bp){var br=this.__Po[bo];

if(!br){return null;
}var bq=this.__Pp.pop()||new qx.ui.form.ListItem();
bq.set({label:br.getLabel(),icon:br.getIcon()});

if(this.__Pq.isItemSelected(bo)){bq.addState(S);
}else{bq.removeState(S);
}bq.setUserData(O,bo);
return bq;
},poolCellWidget:function(g){this.__Pp.push(g);
},getValue:function(){var bs=this.getSelection();
var bt=[];
var bu;

for(var i=0,l=bs.length;i<l;i++){bu=bs[i].getValue();
if(bu==null){bu=bs[i].getLabel();
}bt.push(bu);
}return bt.join(L);
},setValue:function(bx){var bA=bx.split(L);
var bz=[];
var by;

for(var i=0,l=bA.length;i<l;i++){by=this.findItem(bA[i]);

if(by){bz.push(by);
}}this.setSelection(bz);
},getChildren:function(){return this.__Po;
},hasChildren:function(){return this.__Po.length>0;
},_addHelper:function(bl){this.fireDataEvent(V,bl);
bl.addListener(M,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},_removeHelper:function(H){this.fireDataEvent(T,H);
H.removeListener(M,this._onChangeItemHeight,this);
qx.ui.core.queue.Widget.add(this);
},add:function(s){this.__Po.push(s);
this._addHelper(s);
return this;
},remove:function(f){qx.lang.Array.remove(this.__Po,f);
this._removeHelper(f);
return this;
},removeAll:function(){for(var i=0,j=this.__Po.length;i<j;i++){this._removeHelper(this.__Po[i]);
}this.__Po=[];
},indexOf:function(bB){return this.__Po.indexOf(bB);
},addAt:function(c,d){qx.lang.Array.insertAt(this.__Po,c,d);
this._addHelper(c);
},addBefore:function(a,b){qx.lang.Array.insertBefore(this.__Po,a,b);
this._addHelper(a);
},addAfter:function(x,y){qx.lang.Array.insertAfter(this.__Po,x,y);
this._addHelper(x);
},removeAt:function(w){this.__Po.splice(w,1);
this._removeHelper(this.__Po[w]);
},__Pr:function(F){var G=this.__Po.indexOf(F);

if(G<0){return null;
}else{return G;
}},__Ps:function(bv){var bw=this.__Po[bv];
return bw;
},__Pt:function(u){var v=[];

for(var i=0;i<u.length;i++){v.push(this.__Pr(u[i]));
}return v;
},__Pu:function(bm){var bn=[];

for(var i=0;i<bm.length;i++){bn.push(this.__Po[bm[i]]);
}return bn;
},selectAll:function(){this.__Pq.selectAll();
},setSelected:function(I){this.__Pq.selectItem(this.__Pr(I));
},isSelected:function(z){return this.__Pq.isItemSelected(this.__Pr(z));
},addToSelection:function(A){this.__Pq.addItem(this.__Pr(A));
},removeFromSelection:function(n){this.__Pq.removeItem(this.__Pr(n));
},selectRange:function(bj,bk){this.__Pq.selectItemRange(this.__Pr(bj),this.__Pr(bk));
},resetSelection:function(){this.__Pq.clearSelection();
},setSelection:function(t){this.__Pq.replaceSelection(this.__Pt(t));
},getSelectedItem:function(){return this.__Po[this.getSelected()];
},getSelected:function(){return this.__Po[this.__Pq.getSelectedItem()];
},getSelection:function(){return this.__Pu(this.__Pq.getSelection());
},getSortedSelection:function(){return this.__Pu(this.__Pq.getSortedSelection());
},isSelectionEmpty:function(){return this.__Pq.isSelectionEmpty();
},getSelectionContext:function(){return this.__Pq.getSelectionContext();
},getSelectables:function(){return this.__Pu(this.__Pq.getSelectables());
},_applySelectionMode:function(D,E){this.__Pq.setMode(D);
},_applyDragSelection:function(q,r){this.__Pq.setDrag(q);
},_applyQuickSelection:function(o,p){this.__Pq.setQuick(o);
}}});
})();
