...
this.getRoot().add(W,{left:10,top:50});
W.setLayout(new qx.ui.layout.Grid(0,3));
W.add(new qx.ui.basic.Label(A),{row:0,column:0});
W.add(new qx.ui.basic.Label(v),{row:1,column:0});
W.add(new qx.ui.basic.Label(b),{row:2,column:0});
var Q=new qx.ui.form.TextField();
W.add(Q,{row:0,column:1});
var S=new qx.ui.form.TextField();
W.add(S,{row:1,column:1});
var T=new qx.ui.form.Spinner(0,0,1000);
W.add(T,{row:2,column:1});
var U=new qx.data.controller.Object();
U.addTarget(Q,k,D,true);
U.addTarget(S,k,E,true);
U.addTarget(T,k,C,true);
var V=new demobrowser.demo.data.store.Gears();
V.bind(h,U,h);
...
}this.__Mp=this.initDB();
this.__Mq();
},members:{__Mq:function(I){var J=this.getPerson();
this._marshaler.toClass(J,true);
this.setModel(this._marshaler.toModel(J));
this.getModel().addListener(t,this.__Mr,this);
this.fireDataEvent(c,this.getModel());
},__Mr:function(G){var H=this.getModel();
this.update(H.getFirstname(),H.getLastname(),H.getAge());
},update:function(K,L,M){if(!google.gears.factory||!this.__Mp){return;
}this.__Mp.execute(w);
this.__Mp.execute(l,[K,L,M]);
},getPerson:function(){var O={};
O.firstname=i;
O.lastname=i;
O.age=0;

if(!google.gears.factory||!this.__Mp){return O;
}var P=this.__Mp.execute(x);

if(P.isValidRow()){O.firstname=P.field(0);
O.lastname=P.field(1);
O.age=parseInt(P.field(2));
}P.close();
return O;
},initDB:function(){if(window.google&&google.gears){try{var X=google.gears.factory.create(m);

if(X){X.open(z);
X.execute(n+r);
}}catch(N){this.error('Could not create database: '+N.message);
}}return X;
},initGears:function(){if(window.google&&google.gears){return;
}var F=null;
if(typeof GearsFactory!=j){F=new GearsFactory();
}else{try{F=new ActiveXObject(y);
if(F.getBuildInfo().indexOf(p)!=-1){F.privateSetGlobalObject(this);
}}catch(e){if((typeof navigator.mimeTypes!=j)&&navigator.mimeTypes[g]){F=document.createElement(u);
F.style.display=q;
F.width=0;
F.height=0;
F.type=g;
