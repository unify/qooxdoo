...
},createTable:function(){var R=this.createRandomRows(50);
var Q=this._tableModel=new qx.ui.table.model.Simple();
Q.setColumns([l,b,h,g]);
Q.setData(R);
Q.setColumnEditable(1,true);
Q.setColumnEditable(2,true);
Q.setColumnSortable(3,false);
var S=new qx.ui.table.Table(Q);
S.set({width:600,height:400});
S.getSelectionModel().setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
var P=S.getTableColumnModel();
P.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
P.setHeaderCellRenderer(2,new qx.ui.table.headerrenderer.Icon(a,h));
return S;
...
H.addListener(k,function(A){var B=this.createRandomRows(10);
this._tableModel.addRows(B);
this.info("5 rows added");
},this);
J.add(H);
H=new qx.ui.toolbar.Button(w,r);
H.addListener(k,function(U){var V=this._tableModel.getRowCount();
this._tableModel.removeRows(V-5,5);
...
H.addListener(k,function(O){this._table.resetSelection();
},this);
J.add(H);
H=new qx.ui.toolbar.Button(z);
H.addListener(k,function(T){this._table.resetCellFocus();
},this);
J.add(H);
var K=new qx.ui.toolbar.CheckBox(p);
K.addListener(d,function(W){this._table.setShowCellFocusIndicator(!K.isValue());
},this);
J.add(K);
H=new qx.ui.toolbar.Button(n,c);
H.addListener(k,function(C){var D=[];
this._table.getSelectionModel().iterateSelection(function(N){D.push(N+i);
});
this.showDialog(m+D.join(q));
},this);
J.add(H);
return F;
