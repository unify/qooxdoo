...
(function(){var m="",l="selected",k="cell",j="layer",h="icon",g="#f3f3f3",f="label",d="Gallery (HTML - divs)",c="__Qv",b="manager",Q="default",P="demobrowser.demo.virtual.Gallery",O="demobrowser.demo.virtual.WidgetGallery",N="text-align: center;",M="auto",L="color: white; background-color: #00398D;",K="user-desktop.png",J="<img src='",I="'></img>",H="group",t="resize",u="folder.png",r="vertical",s="multi",p="network-server.png",q="user-trash.png",n="off",o="top",v="icon/32/places/",w="horizontal",z="demobrowser.demo.virtual.AbstractGallery",y="abstract",B="position: absolute;",A="demobrowser.demo.virtual.HtmlGallery",D="Gallery (widgets)",C="<br>",x="Icon #",G="network-workgroup.png",F="text-selected",E="demobrowser.demo.virtual.GalleryCell";
...
new demobrowser.demo.virtual.WidgetGallery(D);
var bF=new demobrowser.demo.virtual.HtmlGallery(d);
bF.moveTo(400,50);
}}});
qx.Class.define(z,{extend:qx.ui.window.Window,type:y,construct:function(bJ){qx.ui.window.Window.call(this,bJ);
...
var bK=this._createScroller();
bK.set({scrollbarX:n,scrollbarY:M});
bK.getPane().addListener(t,this._onPaneResize,this);
this.add(bK);
this.manager=new qx.ui.virtual.selection.CellRectangle(bK.getPane(),this).set({mode:s,drag:true});
this.manager.attachMouseEvents();
this.manager.attachKeyEvents(bK);
},members:{getItemData:function(R,S){return this.items[R*this.itemPerLine+S];
},_createScroller:function(){},isItemSelectable:function(bE){return !!this.getItemData(bE.row,bE.column);
},styleSelectable:function(Y,ba,bb){},_onPaneResize:function(e){var bN=e.getTarget();
var bL=e.getData().width;
var bM=Math.floor(bL/this.itemWidth);

if(bM==this.itemsPerLine){return;
}this.itemPerLine=bM;
var bO=Math.ceil(this.itemCount/bM);
bN.getColumnConfig().setItemCount(bM);
bN.getRowConfig().setItemCount(bO);
},_generateItems:function(bc){var bf=[];
var bi=[u,q,p,G,K];
var bg=qx.util.AliasManager.getInstance();
var be=qx.util.ResourceManager.getInstance();

for(var i=0;i<bc;i++){var bj=v+bi[Math.floor(Math.random()*bi.length)];
var bd=bg.resolve(bj);
var bh=be.toUri(bd);
bf[i]={label:x+(i+1),icon:bj,resolvedIcon:bh};
}return bf;
}},destruct:function(){this.items=null;
this._disposeObjects(b);
}});
qx.Class.define(E,{extend:qx.ui.virtual.cell.AbstractWidget,members:{_createWidget:function(){var bP=new qx.ui.basic.Atom().set({iconPosition:o});
bP.getChildControl(f).set({padding:[0,4]});
bP.getChildControl(h).set({padding:4});
return bP;
},updateData:function(U,V){U.set({icon:V.icon,label:V.label});
},updateStates:function(bQ,bR){var bT=bQ.getChildControl(f);
var bS=bQ.getChildControl(h);

if(bR.selected){bT.setDecorator(l);
bT.setTextColor(F);
bS.setDecorator(H);
}else{bT.resetDecorator();
bT.resetTextColor();
bS.resetDecorator();
}}}});
qx.Class.define(O,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(T){demobrowser.demo.virtual.AbstractGallery.call(this,T);
this.__Qv=new demobrowser.demo.virtual.GalleryCell();
},members:{_createScroller:function(){var bG=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.WidgetCell(this);
bG.getPane().addLayer(this.layer);
new qx.ui.virtual.behavior.Prefetch(bG,{minLeft:0,maxLeft:0,minRight:0,maxRight:0,minAbove:200,maxAbove:300,minBelow:600,maxBelow:800}).set({interval:500});
return bG;
},styleSelectable:function(bp,bq,br){if(bq!==l){return;
}var bs=this.layer.getChildren();

for(var i=0;i<bs.length;i++){var bt=bs[i];
var bu=bt.getUserData(k);

if(bp.row!==bu.row||bp.column!==bu.column){continue;
}
if(br){this.__Qv.updateStates(bt,{selected:1});
}else{this.__Qv.updateStates(bt,{});
}}},getCellWidget:function(bv,bw){var bA=this.getItemData(bv,bw);

if(!bA){return null;
}var bx={row:bv,column:bw};
var by={};

if(this.manager.isItemSelected(bx)){by.selected=true;
}var bz=this.__Qv.getCellWidget(bA,by);
bz.setUserData(k,bx);
return bz;
},poolCellWidget:function(a){this.__Qv.pool(a);
}},destruct:function(){this._disposeObjects(c,j);
}});
qx.Class.define(A,{extend:demobrowser.demo.virtual.AbstractGallery,construct:function(W){demobrowser.demo.virtual.AbstractGallery.call(this,W);
var X=qx.theme.manager.Font.getInstance().resolve(Q).getStyles();
this._fontCss=qx.bom.element.Style.compile(X);
},members:{_createScroller:function(){var bI=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new qx.ui.virtual.layer.HtmlCell(this);
bI.getPane().addLayer(this.layer);
var bH=new qx.ui.virtual.layer.GridLines(w,g);
bI.getPane().addLayer(bH);
var bH=new qx.ui.virtual.layer.GridLines(r,g);
bI.getPane().addLayer(bH);
return bI;
},_onPaneResize:function(e){demobrowser.demo.virtual.AbstractGallery.prototype._onPaneResize.call(this,e);
this.manager.clearSelection();
},styleSelectable:function(bB,bC,bD){this.layer.updateLayerData();
},getCellProperties:function(bk,bl){var bo=this.getItemData(bk,bl);

if(!bo){return m;
}var bn=this.manager.isItemSelected({row:bk,column:bl});
var bm=bn?L:m;
return {style:[B,N,this._fontCss,bm].join(m),content:[J,bo.resolvedIcon,I,C,bo.label].join(m)};
