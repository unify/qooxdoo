...
(function(){var o="execute",n="changeValue",m="value",l="icon/22/actions/edit-undo.png",k="Change ID sort method",j="Disabled ctx menu",h="icon/22/status/dialog-information.png",g="On",f="Change row with ID 10",d="Default ctx menu #2",G="Remove 5 rows",F="Add 10 rows",E="Context-menu selectable",D="",C="_tableModel",B="Table",A="icon/22/actions/list-remove.png",z="Whether the the first visible row should ",y=", ",x="demobrowser.demo.table.Table_Context_Menu",v="Default ctx menu #1",w="icon/22/actions/list-add.png",t="Selected rows:<br>",u="Keep first row",r="Off",s="be rendered completely when scrolling.",p="Show selection",q="Demonstrate use of alternate sorting algorithm.";
qx.Class.define(x,{extend:demobrowser.demo.table.TableDemo,members:{main:function(){qx.Class.include(qx.ui.table.Table,qx.ui.table.MTableContextMenu);
demobrowser.demo.table.TableDemo.prototype.main.call(this);
this.getRoot().setNativeContextMenu(true);
},getCaption:function(){return B;
},createTable:function(){var M=this.createRandomRows(50);
var L=this._tableModel=new qx.ui.table.model.Simple();
L.setColumns([v,d,j,E]);
L.setData(M);
var N=new qx.ui.table.Table(L);
N.set({width:600,height:400,decorator:null});
var K=N.getTableColumnModel();
K.setDataCellRenderer(3,new qx.ui.table.cellrenderer.Boolean());
N.setContextMenuHandler(3,this._contextMenuHandlerBoolean);
N.setContextMenuHandler(2,this._contextMenuHandlerNull);
return N;
},_contextMenuHandlerBoolean:function(bb,bc,bd,be,bf){var bg;

for(var i=0;i<=1;i++){bg=new qx.ui.menu.Button(i?g:r);
bg.setUserData(m,i?true:false);
bg.addListener(o,function(e){be.setValue(bb,bc,this.getUserData(m));
});
bf.add(bg);
}return true;
...
T.add(S);
R=new qx.ui.toolbar.Button(f,l);
R.addListener(o,function(O){var P=this.createRandomRows(1);

for(var i=1;i<this._tableModel.getColumnCount();i++){this._tableModel.setValue(i,10,P[0][i]);
}this.info("Row 10 changed");
},this);
S.add(R);
R=new qx.ui.toolbar.Button(F,w);
R.addListener(o,function(bo){var bp=this.createRandomRows(10);
this._tableModel.addRows(bp);
this.info("10 rows added");
},this);
S.add(R);
R=new qx.ui.toolbar.Button(G,A);
R.addListener(o,function(H){var I=this._tableModel.getRowCount();
this._tableModel.removeRows(I-5,5);
this.info("5 rows removed");
},this);
S.add(R);
R=new qx.ui.toolbar.Button(p,h);
R.addListener(o,function(bh){var bi=[];
Q.getSelectionModel().iterateSelection(function(ba){bi.push(ba+D);
});
this.showDialog(t+bi.join(y));
},this);
S.add(R);
S=new qx.ui.toolbar.Part();
T.add(S);
...
U.set({value:Q.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(z+s)});
U.addListener(n,function(J){Q.setKeepFirstVisibleRowComplete(this.getValue());
},U);
S.add(U);
U=new qx.ui.toolbar.CheckBox(k);
U.set({value:Q.getKeepFirstVisibleRowComplete(),toolTip:new qx.ui.tooltip.ToolTip(q)});
U.addListener(n,function(a){if(a.getData()){var c=function(V,W){var X=V[arguments.callee.columnIndex];
...
var b=function(bq,br){var bs=bq[arguments.callee.columnIndex];
...
Q.getTableModel().setSortMethods(0,{ascending:c,descending:b});
...
this.__OB=[];
},members:{__OB:null,setContextMenuHandler:function(i,j){this.__OB[i]=j;
},getContextMenuHandler:function(k){return this.__OB[k];
