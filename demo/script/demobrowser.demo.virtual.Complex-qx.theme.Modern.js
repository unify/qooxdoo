...
this.__Qs=[];
},members:{_pool:null,__Qs:null,getCellData:function(x,y){if(!this.__Qs[x]){this.__Qs[x]=[];
}
if(!this.__Qs[x][y]){this.__Qs[x][y]={label:this.__Qt(),icon:this.__Qu()};
}return this.__Qs[x][y];
},getCellWidget:function(A,B){var C;

if(B%2==0){C=this._pool.atom.pop();

if(!C){C=new qx.ui.basic.Atom();
C.addListener(c,function(){var z=this.__Qu();
C.setIcon(z);
this.__Qs[A][B].icon=z;
},this);
}C.set(this.getCellData(A,B));
}else{C=this._pool.checkbox.pop();

if(!C){C=new qx.ui.form.CheckBox();
C.addListener(f,function(){this.setLabel(this.getLabel()==p?C.getUserData(q)+r+C.getUserData(o):p);
},C);
}C.set({value:A%2==0,label:A+r+B});
}C.setUserData(q,A);
C.setUserData(o,B);
return C;
},poolCellWidget:function(s){if(s.classname==a){this._pool.atom.push(s);
}else{this._pool.checkbox.push(s);
}},__Qt:function(){var name=m;

for(var j=0;j<10;j++){name+=String.fromCharCode(Math.floor(Math.random()*25)+65);
}return name;
},__Qu:function(){var t=n;
var v=k;
var w=[l,g,h,b,d];
var u=Math.floor(Math.random()*4);
return (t+16+e+v+w[u]);
}},destruct:function(){this._pool=this.__Qs=null;
}});
})();
