...
},getNewWidget:function(){var q=new qx.ui.core.Widget().set({width:50,height:50,backgroundColor:j});
q.addListener(h,function(e){if(this._active==q){return;
}var m=[];
var k=0.3;

if(this._active){this._active.set({backgroundColor:j,width:50,height:50});
}q.set({backgroundColor:b});
var n=q.getBounds();
m.push(new demobrowser.demo.layout.Grid_Animated_Property(q,f).set({from:n.width,to:200,duration:k,transition:i}));
m.push(new demobrowser.demo.layout.Grid_Animated_Property(q,d).set({from:n.height,to:200,duration:k,transition:i}));
var l=new qx.fx.effect.core.Parallel(m[0],m[1]);
l.start();
this._active=q;
},this);
return q;
},getAnimGrid:function(){var o=new qx.ui.container.Composite().set({decorator:a,backgroundColor:g,width:500,height:500});
var p=new qx.ui.layout.Grid();
p.setSpacing(3);
o.setLayout(p);
this._active=null;

for(var x=0;x<7;x++){p.setColumnFlex(x,1);
p.setRowFlex(x,1);

for(var y=0;y<7;y++){o.add(this.getNewWidget(),{row:y,column:x});
}}return o;
}}});
})();
