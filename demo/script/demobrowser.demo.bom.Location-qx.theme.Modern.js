...
var x=document.getElementById(h).getElementsByTagName(c);

for(var i=0,l=x.length;i<l;i++){x[i].onclick=demobrowser.demo.bom.Location.jump;
}}},statics:{num:function(o,p){return parseInt(qx.bom.element.Style.get(o,p,qx.bom.element.Style.COMPUTED_MODE,false),10);
...
var u=s.substr(s.indexOf(g)+1);
var r=document.getElementById(u);
var location=qx.bom.element.Location.get(r,document.getElementById(f).value);
var t=document.getElementById(m);
t.style.left=location.left+n;
t.style.top=location.top+n;

if(qx.bom.client.Feature.BORDER_BOX){t.style.width=(location.right-location.left)+n;
t.style.height=(location.bottom-location.top)+n;
}else{t.style.width=(location.right-location.left-q(t,d)-q(t,b))+n;
t.style.height=(location.bottom-location.top-q(t,e)-q(t,j))+n;
}return false;
